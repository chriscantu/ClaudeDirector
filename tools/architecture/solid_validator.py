...
class SOLIDValidator:
    ...
    def _check_interface_segregation(self, tree: ast.AST, file_path: Path):
        ...
        for class_info in class_visitor.classes:
            ...
            abstract_methods = [m for m in class_info['methods'] 
                             if any(d.id == 'abstractmethod' for d in m.get('decorators', []) 
    ...
    def _check_dependency_inversion(self, tree: ast.AST, file_path: Path):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
                ...
                if (class_name.endswith('Engine') or class_name.endswith('Service') or 
                    class_name.endswith('Manager')) and not class_name.startswith('Mock'):
                    
                    # Allow factory methods and dependency injection patterns
                    if not self._is_factory_pattern(node) and not self._is_dependency_injection(node):
                        self.violations.append(Violation(
                            file_path=str(file_path),
                            line_number=node.lineno,
                            principle="DIP",
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            for pattern in self.hardcode_patterns:
                ...
                for match in matches:
                    ...
                    is_allowed = any(re.search(allowed, match.group(), re.IGNORECASE) 
