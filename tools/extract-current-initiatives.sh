...
if [[ -n "$JIRA_API_TOKEN" ]]; then
    ...
    USER_RESPONSE=$(curl -s -u "***REMOVED***:$JIRA_API_TOKEN" \
        -H "Accept: application/json" \
        "https://***REMOVED***/rest/api/3/myself" | \
...
extract_active_initiatives() {
    ...
    curl -s -u "***REMOVED***:$JIRA_API_TOKEN" \
        -H "Accept: application/json" \
        "https://***REMOVED***/rest/api/3/search?jql=$(echo "$ACTIVE_QUERY" | sed 's/ /%20/g')&maxResults=200&fields=summary,status,priority,assignee,project,labels,components,fixVersions,description,updated,created" | \
...
extract_strategic_epics() {
    ...
    curl -s -u "***REMOVED***:$JIRA_API_TOKEN" \
        -H "Accept: application/json" \
        "https://***REMOVED***/rest/api/3/search?jql=$(echo "$STRATEGIC_QUERY" | sed 's/ /%20/g')&maxResults=100&fields=summary,status,priority,assignee,project,labels,components,fixVersions,description,updated,created,timeestimate,timeoriginalestimate" | \
...
extract_recent_completed() {
    ...
    curl -s -u "***REMOVED***:$JIRA_API_TOKEN" \
        -H "Accept: application/json" \
        "https://***REMOVED***/rest/api/3/search?jql=$(echo "$COMPLETED_QUERY" | sed 's/ /%20/g')&maxResults=100&fields=summary,status,priority,assignee,project,labels,components,resolutiondate,updated" | \
...
EOF
        ...
        jq -r "select(.fields.project.key == \"$project\") | \"- [\(.key)](https://***REMOVED***/browse/\(.key)): \(.fields.summary)\"" "${OUTPUT_DIR}/active_initiatives_raw.json" >> "$ANALYSIS_FILE"
...
EOF
    ...
    jq -r 'select(.fields.labels[]? | test("platform|foundation|architecture|mfe|design-system"; "i")) | "- [\(.key)](https://***REMOVED***/browse/\(.key)): \(.fields.summary) - \(.fields.status.name) - \(.fields.priority.name // "No Priority")"' "${OUTPUT_DIR}/strategic_epics_raw.json" >> "$ANALYSIS_FILE"
...
EOF
    ...
    jq -r 'select(.fields.labels[]? | test("quality|monitoring|slo|observability"; "i")) | "- [\(.key)](https://***REMOVED***/browse/\(.key)): \(.fields.summary) - \(.fields.status.name) - \(.fields.priority.name // "No Priority")"' "${OUTPUT_DIR}/strategic_epics_raw.json" >> "$ANALYSIS_FILE"
...
EOF
    ...
    jq -r 'select(.fields.labels[]? | test("platform|foundation|architecture|quality|monitoring"; "i") | not) | "- [\(.key)](https://***REMOVED***/browse/\(.key)): \(.fields.summary) - \(.fields.status.name) - \(.fields.priority.name // "No Priority")"' "${OUTPUT_DIR}/strategic_epics_raw.json" >> "$ANALYSIS_FILE"
...
$(jq -r 'select(.fields.priority.name == "High" or .fields.priority.name == "Highest") | "- [\(.key)](https://***REMOVED***/browse/\(.key)): \(.fields.summary) - \(.fields.project.key)"' "${OUTPUT_DIR}/active_initiatives_raw.json")

### At Risk / Blocked Initiatives
$(jq -r 'select(.fields.status.name | test("blocked|risk"; "i")) | "- [\(.key)](https://***REMOVED***/browse/\(.key)): \(.fields.summary) - \(.fields.status.name)"' "${OUTPUT_DIR}/active_initiatives_raw.json")
