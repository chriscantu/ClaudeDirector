...
extract_l2_initiatives() {
    ...
    curl -s -u "***REMOVED***:$JIRA_API_TOKEN" \
        -H "Accept: application/json" \
        "https://***REMOVED***/rest/api/3/search?jql=$(echo "$L2_QUERY" | sed 's/ /%20/g')&maxResults=100&fields=summary,status,priority,assignee,project,labels,components,fixVersions,description,updated,created,cf[18272],cf[18280],cf[18281],cf[18282]" | \
...
extract_l1_context() {
    ...
    curl -s -u "***REMOVED***:$JIRA_API_TOKEN" \
        -H "Accept: application/json" \
        "https://***REMOVED***/rest/api/3/search?jql=$(echo "$L1_QUERY" | sed 's/ /%20/g')&maxResults=50&fields=summary,status,priority,assignee,project,labels,components,parent,updated,created" | \
...
**Source**: ***REMOVED*** Jira PI project - L2 business initiatives
...
EOF
    ...
    if [[ -f "${OUTPUT_DIR}/l2_strategic_raw.json" && $(jq -s length "${OUTPUT_DIR}/l2_strategic_raw.json") -gt 0 ]]; then
        jq -r '. | "### [\(.key)](https://***REMOVED***/browse/\(.key)): \(.fields.summary)
...
EOF
    ...
    if [[ -f "${OUTPUT_DIR}/l1_context_raw.json" && $(jq -s length "${OUTPUT_DIR}/l1_context_raw.json") -gt 0 ]]; then
        jq -r '. | "- [\(.key)](https://***REMOVED***/browse/\(.key)): \(.fields.summary) - \(.fields.status.name) - \(.fields.priority.name // "No Priority")"' "${OUTPUT_DIR}/l1_context_raw.json" >> "$L2_ANALYSIS_FILE"
    else
...
## Data Sources
- *****REMOVED*** Jira PI Project**: L2 and L1 initiatives for UI Foundations division
