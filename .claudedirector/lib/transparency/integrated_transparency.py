...
class MultiPersonaResponseFormatter:
    ...
    def format_collaborative_response(
        self,
        primary_persona: str,
    ...
    def format_sequential_consultation(
        self,
        primary_persona: str,
        ...
        consulting_name = (
            consulting_persona.split("|")[0].strip()
            if "|" in consulting_persona
            else consulting_persona
        )
...
Building on {consulting_name}'s insights, {integration_response}"""
    ...
    def format_persona_handoff(
        self, transition: PersonaTransition, response: str
    ) -> str:
...
class IntegratedTransparencySystem:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.framework_attribution_enabled = self.config.get(
    ...
    def create_multi_persona_context(
        self,
        primary_persona: str,
        consulting_personas: List[str] = None,
    ...
    def apply_transparency(self, context: TransparencyContext, response: str) -> str:
        ...
        if self.framework_attribution_enabled:
            context.detected_frameworks = (
                self.framework_middleware.detect_frameworks_used(response)
            )
            if context.detected_frameworks:
                self.performance_stats["frameworks_detected"] += len(
                    context.detected_frameworks
                )
    ...
    def create_transparency_summary(
        self, context: TransparencyContext
    ) -> Dict[str, Any]:
    ...
    def apply_multi_persona_transparency(
        self, multi_context: MultiPersonaContext, response: str
    ) -> str:
    ...
    def _apply_collaborative_transparency(
        self, multi_context: MultiPersonaContext, response: str
    ) -> str:
        ...
        enhanced_response = self.multi_persona_formatter.format_collaborative_response(
            multi_context.primary_persona, consultant_responses, integration_summary
        )
    ...
    def _apply_sequential_transparency(
        self, multi_context: MultiPersonaContext, response: str
    ) -> str:
        ...
        if multi_context.consulting_personas:
            ...
            consultation_response = multi_context.consultation_results.get(
                consulting_persona, ""
            )

            # Phase 2: Apply multi-persona transparency enhancements
            enhanced_response = (
                self.multi_persona_formatter.format_sequential_consultation(
    ...
    def _apply_handoff_transparency(
        self, multi_context: MultiPersonaContext, response: str
    ) -> str:
        ...
        if multi_context.consulting_personas:
            ...
            # Phase 2: Apply multi-persona transparency enhancements
            enhanced_response = self.multi_persona_formatter.format_persona_handoff(
                transition, response
            )
    ...
    def wrap_multi_persona_response(
        self,
        multi_context: MultiPersonaContext,
        response: str,
        ...
        enhanced_response = self.apply_multi_persona_transparency(
            multi_context, enhanced_response
        )
        ...
        if transparency_context and transparency_context.has_enhancements:
            ...
            if (
                self.mcp_disclosure_enabled
                and transparency_context.mcp_context.has_mcp_calls()
            ):
                enhanced_response = self.mcp_middleware.wrap_persona_response(
                    multi_context.primary_persona,
                    enhanced_response,
                    transparency_context.mcp_context,
                )

            # Apply framework attribution if present
            if (
                self.framework_attribution_enabled
                and transparency_context.detected_frameworks
            ):
                enhanced_response = self.framework_middleware.add_framework_attribution(
                    multi_context.primary_persona,
                    enhanced_response,
                    transparency_context.detected_frameworks,
                )

        return enhanced_response

    def _apply_multi_persona_mcp_transparency(
        self, multi_context: MultiPersonaContext, response: str
    ) -> str:
        ...
        mcp_disclosure = self._format_multi_persona_mcp_disclosure(
            multi_context, mcp_calls_summary
        )
        if mcp_disclosure:
            return f"{response}\n\n{mcp_disclosure}"
    ...
    def _apply_multi_persona_framework_attribution(
        self, multi_context: MultiPersonaContext, response: str
    ) -> str:
    ...
    def _format_multi_persona_mcp_disclosure(
        self, multi_context: MultiPersonaContext, mcp_calls_summary: Dict[str, Any]
    ) -> str:
        ...
        for persona, servers in mcp_calls_summary.items():
            if servers:
                persona_header = self.multi_persona_formatter._get_persona_header(
                    persona
                )
        ...
        lines.append("---")
        lines.append(
            "**Enhanced Analysis**: Cross-functional insights powered by strategic frameworks and collaborative intelligence."
        )

        return "\n".join(lines)

    def _format_multi_persona_framework_attribution(
        self, multi_context: MultiPersonaContext, frameworks_used: Dict[str, List[str]]
    ) -> str:
        ...
        lines.append("---")
        lines.append(
            "**Framework Attribution**: This cross-functional analysis combines proven strategic frameworks, adapted through collaborative expertise."
        )

        return "\n".join(lines)

    def coordinate_multi_persona_mcp_calls(
        self, transparency_contexts: Dict[str, TransparencyContext]
    ) -> Dict[str, Any]:
    ...
    def coordinate_multi_persona_frameworks(
        self, transparency_contexts: Dict[str, TransparencyContext]
    ) -> Dict[str, List[str]]:
        ...
        for persona, context in transparency_contexts.items():
            if context.detected_frameworks:
                framework_names = [
                    f.framework_name for f in context.detected_frameworks
                ]
                framework_summary[persona] = framework_names

        return framework_summary

    def enhance_multi_persona_context(
        self,
        multi_context: MultiPersonaContext,
        transparency_contexts: Dict[str, TransparencyContext],
    ) -> MultiPersonaContext:
        """Enhance multi-persona context with transparency information (Phase 2)"""
        # Add MCP calls summary
        multi_context.mcp_calls_summary = self.coordinate_multi_persona_mcp_calls(
            transparency_contexts
        )

        # Add frameworks used summary
        multi_context.frameworks_used = self.coordinate_multi_persona_frameworks(
            transparency_contexts
        )

        return multi_context

    def create_enhanced_multi_persona_response(
        self,
        primary_persona: str,
        consulting_personas: List[str],
        consultation_results: Dict[str, str],
        ...
        if transparency_contexts:
            multi_context = self.enhance_multi_persona_context(
                multi_context, transparency_contexts
            )
    ...
    def _update_performance_stats(
        self, transparency_overhead: float, has_enhancements: bool
    ):
...
class PersonaTransparencyDecorator:
    ...
    def __call__(self, persona_func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            persona = kwargs.get(
                "persona", getattr(persona_func, "persona_name", "unknown")
            )
...
def create_transparency_system(
