...
class ConversationalAnalyticsWorkflow:
    ...
    async def process_chat_query(
        ...
        try:
            ...
            parsed_query, data_response = await self.data_manager.process_conversational_query(
                query_text, context
            )
            ...
            if data_response.success:
                ...
                visualization_result = VisualizationResult(
                    success=False,
                    html_output=self._generate_error_response(data_response.error_message),
                    chart_type="error",
                    persona=persona,
                    generation_time=0.0,
                    file_size_bytes=0,
            ...
            return ConversationalAnalyticsResult(
                ...
                pipeline_metadata={
                    ...
                    "visualization_ms": int(visualization_result.generation_time * 1000),
    ...
    def _update_pipeline_metrics(
        ...
        self.pipeline_metrics["avg_latency_ms"] = (
            (self.pipeline_metrics["avg_latency_ms"] * (total_queries - 1) + total_latency_ms) / total_queries
        )
        
        self.pipeline_metrics["avg_data_fetch_ms"] = (
            (self.pipeline_metrics["avg_data_fetch_ms"] * (total_queries - 1) + data_fetch_ms) / total_queries
        )
        
        self.pipeline_metrics["avg_visualization_ms"] = (
            (self.pipeline_metrics["avg_visualization_ms"] * (total_queries - 1) + visualization_ms) / total_queries
        )
    ...
    async def get_pipeline_health(self) -> Dict[str, Any]:
        ...
        if self.pipeline_metrics["total_queries"] > 0:
            success_rate = (
                self.pipeline_metrics["successful_queries"] / self.pipeline_metrics["total_queries"]
            )
        
        return {
            "pipeline_name": self.name,
            "version": self.version,
            "health_status": "healthy" if success_rate > 0.95 else "warning" if success_rate > 0.8 else "critical",
            "metrics": {
                **self.pipeline_metrics,
