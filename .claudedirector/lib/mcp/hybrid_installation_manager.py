...
class HybridInstallationManager:
                ...
                with open(stats_file, 'r') as f:
                    ...
                    for server_name, stats_data in data.items():
                        stats[server_name] = UsageStats(
                            ...
                            last_hint_shown=datetime.fromisoformat(stats_data['last_hint_shown']) if stats_data.get('last_hint_shown') else None,
                            total_startup_time_saved=stats_data.get('total_startup_time_saved', 0.0)
                        )
    ...
    def _save_usage_stats(self):
        ...
        for server_name, stats in self.usage_stats.items():
            data[server_name] = {
                ...
                'last_hint_shown': stats.last_hint_shown.isoformat() if stats.last_hint_shown else None,
    ...
    def try_installation_command(self, cmd: InstallationCommand, timeout: int = 10) -> InstallationResult:
    ...
    def _track_usage(self, server_name: str, installation_type: str, startup_time: float):
        ...
        if not self.config.get("hybrid_installation", {}).get("track_installation_performance", True):
            return
        ...
        if installation_type == "permanent":
            ...
            stats.total_startup_time_saved += max(0, startup_time - 0.46)  # Assume permanent would be ~0.46s
    ...
    def _show_installation_message(self, server_name: str, result: InstallationResult):
        ...
        if result.installation_type == "permanent" and result.performance_benefit:
            msg = messages.get("permanent_found", "⚡ Using optimized MCP server: {server} ({benefit})")
    ...
    def _check_performance_hint(self, server_name: str, installation_type: str):
        ...
        if (stats.temporary_uses >= threshold and
            stats.permanent_uses == 0 and
            (not stats.last_hint_shown or
             (datetime.now() - stats.last_hint_shown).days >= 7)):  # Don't spam hints
    ...
    def _show_performance_hint(self, server_name: str, stats: UsageStats):
        ...
        hint_msg = messages.get("performance_hint",
        ...
        print(f"   You've used {server_name} {stats.temporary_uses} times - permanent installation saves ~{stats.total_startup_time_saved:.1f}s total")
