...
class EnhancedTransparencyEngine:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.performance_mode = self.config.get(
    ...
    def enhance_response(
        ...
        try:
            if self.mode == "full_system":
                return self._enhance_with_full_system(
                    response, user_input, metadata, start_time
                )
            elif self.mode == "bridge_fallback":
                return self._enhance_with_bridge(
                    response, user_input, metadata, start_time
                )
        except Exception as e:
    ...
    def _enhance_with_bridge(
        ...
        # Get summary
        transparency_summary = self.bridge.create_transparency_summary(
        ...
        processing_time = time.time() - start_time
        self._update_stats(
            processing_time, transparency_summary.get("transparency_applied", False), 0
        )
    ...
    def _detect_persona_from_input(self, user_input: str) -> str:
        ...
        if any(
            word in input_lower
            for word in ["architecture", "platform", "scalability", "technical"]
        ):
            return "martin"
        elif any(
            word in input_lower
            for word in ["team", "leadership", "organization", "management"]
        ):
            return "diego"
        elif any(
            word in input_lower for word in ["business", "roi", "investment", "value"]
        ):
            return "alvaro"
...
def get_transparency_engine(
    config: Optional[Dict] = None,
) -> EnhancedTransparencyEngine:
...
def enhance_response_transparency(
...
if __name__ == "__main__":
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        result = engine.enhance_response(test_case["response"], test_case["input"])
    ...
    print(
        f"   Enhanced: {stats['enhanced_requests']} ({stats['enhancement_rate']:.1f}%)"
    )
