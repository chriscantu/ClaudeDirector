...
class StrategicTaskManager:
        ...
        for pattern in file_patterns:
            for file_path in workspace_dir.rglob(pattern):
                try:
                    ...
                    # Detect tasks
                    detected_tasks = self.task_detector.detect_tasks_in_content(
                        content, context
                    )
                    ...
                    if detected_tasks:
                        ...
                        for task in detected_tasks:
                            ...
                            if result["action"] == "created":
                                ...
                                print(
                                    f"   ❓ Review needed: {task['task_text'][:60]}..."
                                )
                            else:
    ...
    def _create_task_from_detection(
        self, task_data: Dict, source_file: Path
    ) -> Optional[int]:
    ...
    def _task_exists(self, task_key: str) -> bool:
        ...
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT id FROM strategic_tasks WHERE task_key = ?", (task_key,)
                )
    ...
    def show_my_tasks(self, status_filter: Optional[str] = None):
        ...
        try:
            with self.get_connection() as conn:
                ...
                for task in tasks:
                    task_id, title, priority, due_date, status, category, direction = (
                        task
                    )
        ...
        try:
            with self.get_connection() as conn:
                ...
                for task in tasks:
                    ...
                    print(
                        f"{priority_emoji} {direction_emoji} [{task_id}] {title[:50]}"
                    )
        ...
        try:
            with self.get_connection() as conn:
                ...
                for follow_up in follow_ups:
                    ...
                    stakeholder_display = (
                        stakeholder_name if stakeholder_name else stakeholder_key
                    )
        ...
        try:
            with self.get_connection() as conn:
                ...
                for review in reviews:
                    review_id, task_data_json, source_file, confidence, created_at = (
                        review
                    )
                    ...
                    print(
                        f"{confidence_emoji} {direction_emoji} [{review_id}] {task_data['task_text'][:60]}"
                    )
    ...
    def _show_task_summary(self):
        ...
        try:
            with self.get_connection() as conn:
                ...
                # Get counts
                cursor.execute(
                    "SELECT COUNT(*) FROM strategic_tasks WHERE status = 'active'"
                )
                active_count = cursor.fetchone()[0]

                cursor.execute(
                    "SELECT COUNT(*) FROM strategic_tasks WHERE status = 'active' AND due_date < date('now')"
                )
                overdue_count = cursor.fetchone()[0]

                cursor.execute(
                    "SELECT COUNT(*) FROM strategic_tasks WHERE follow_up_required = TRUE AND follow_up_date <= date('now') AND status = 'active'"
                )
                followup_count = cursor.fetchone()[0]

                cursor.execute(
                    "SELECT COUNT(*) FROM task_review_queue WHERE status = 'pending'"
                )
                ...
                if overdue_count > 0:
                    print(
                        "   1. Review overdue tasks: python strategic_task_manager.py overdue"
                    )
                if followup_count > 0:
                    print(
                        "   2. Complete follow-ups: python strategic_task_manager.py followups"
                    )
                if review_count > 0:
                    print(
                        "   3. Review detected tasks: python strategic_task_manager.py review"
                    )

                print("   4. View all my tasks: python strategic_task_manager.py list")
                print(
                    "   5. View assigned tasks: python strategic_task_manager.py assigned"
                )
