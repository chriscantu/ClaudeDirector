...
class ROIInvestmentTracker:
    ...
    def create_investment_proposal(self, proposal_data: Dict[str, Any]) -> InvestmentProposal:
        ...
        try:
            ...
            proposal = InvestmentProposal(
                ...
                ongoing_annual_cost=Decimal(str(proposal_data.get('ongoing_annual_cost', 0))),
                ...
                roi_calculation_method=ROICalculationMethod(proposal_data['roi_method']),
                ...
                alternatives_considered=proposal_data.get('alternatives_considered', []),
            ...
            self.logger.info("Investment proposal created",
                           proposal_id=proposal_id,
    ...
    def evaluate_investment_proposal(self, proposal_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info("Investment proposal evaluated",
                           proposal_id=proposal_id,
            ...
            self.logger.error("Investment evaluation failed", proposal_id=proposal_id, error=str(e))
            return {'error': str(e)}
    ...
    def track_investment_performance(self, investment_id: str, period: str) -> ROITracking:
        ...
        try:
            ...
            variance_percentage = (variance_amount / planned_benefits * 100) if planned_benefits > 0 else Decimal('0')
            ...
            success_metrics_achieved, total_metrics = self._assess_success_metrics(proposal)
            ...
            benefit_breakdown = self._breakdown_benefits_by_type(actual_benefits, proposal)
            ...
            self.logger.info("Investment performance tracked",
                           investment_id=investment_id,
                           period=period,
            ...
            self.logger.error("Investment performance tracking failed",
    ...
    def generate_portfolio_summary(self, reporting_period: str) -> InvestmentPortfolioSummary:
        ...
        try:
            active_investments = [
                p for p in self._investment_proposals.values()
                if p.status in [InvestmentStatus.APPROVED, InvestmentStatus.IN_PROGRESS, InvestmentStatus.COMPLETED]
            ]

            # Calculate portfolio metrics
            total_portfolio_value = sum(inv.total_investment for inv in active_investments)
            total_benefits_realized = self._calculate_total_benefits_realized(active_investments, reporting_period)

            # Performance analysis
            performance_metrics = self._analyze_portfolio_performance(active_investments)
            ...
            portfolio_summary = InvestmentPortfolioSummary(
                ...
                recommended_future_investments=strategic_recommendations['future_investments'],
    ...
    def generate_investment_justification_report(self, proposal_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info("Investment justification report generated",
                           proposal_id=proposal_id,
            ...
            self.logger.error("Investment justification report failed",
    ...
    def _calculate_roi_projections(self, proposal_data: Dict[str, Any]) -> Dict[str, Decimal]:
        ...
        if roi_method == ROICalculationMethod.DIRECT_SAVINGS:
            base_annual_benefit = total_investment * Decimal('0.4')  # 40% annual savings
        elif roi_method == ROICalculationMethod.PRODUCTIVITY_GAINS:
            base_annual_benefit = total_investment * Decimal('0.35')  # 35% productivity ROI
        elif roi_method == ROICalculationMethod.RISK_MITIGATION:
            base_annual_benefit = total_investment * Decimal('0.25')  # 25% risk mitigation value
        elif roi_method == ROICalculationMethod.REVENUE_ENABLEMENT:
            base_annual_benefit = total_investment * Decimal('0.6')   # 60% revenue enablement
        else:  # STRATEGIC_VALUE
            base_annual_benefit = total_investment * Decimal('0.3')   # 30% strategic value
    ...
    def _analyze_financial_metrics(self, proposal: InvestmentProposal) -> Dict[str, Any]:
        ...
        payback_score = max(0, 100 - (payback_months / 36) * 100)  # Penalty for long payback
    ...
    def _assess_proposal_risks(self, proposal: InvestmentProposal) -> Dict[str, Any]:
        ...
        if technical_risk:
            mitigation_strategies.append("Conduct technical proof of concept before full implementation")
        if integration_risk:
            mitigation_strategies.append("Plan phased integration with fallback options")
        if adoption_risk:
            mitigation_strategies.append("Develop comprehensive change management and training plan")
        if timeline_risk:
            mitigation_strategies.append("Break implementation into smaller, deliverable phases")
        if budget_risk:
            mitigation_strategies.append("Implement strict budget controls and regular review checkpoints")
        ...
        return {
            ...
            'recommended_contingency_budget': proposal.total_investment * Decimal('0.15')  # 15% contingency
        }
    ...
    def _calculate_strategic_alignment_score(self, proposal: InvestmentProposal) -> Decimal:
        ...
        bonus_score = sum(
            bonus for keyword, bonus in alignment_bonuses.items()
    ...
    def _analyze_portfolio_fit(self, proposal: InvestmentProposal) -> Dict[str, Any]:
        ...
        for inv in active_investments:
            ...
            total_category_investment[category] = total_category_investment.get(category, Decimal('0')) + inv.total_investment

        # Check for over-concentration
        proposed_category = proposal.category.value
        new_category_count = category_counts.get(proposed_category, 0) + 1
        new_category_investment = total_category_investment.get(proposed_category, Decimal('0')) + proposal.total_investment

        total_portfolio_value = sum(inv.total_investment for inv in active_investments) + proposal.total_investment
        category_concentration = (new_category_investment / total_portfolio_value) * 100 if total_portfolio_value > 0 else 0

        # Portfolio balance assessment
        concentration_risk = category_concentration > 40  # More than 40% in one category
        diversification_benefit = new_category_count == 1  # New category adds diversification

        return {
            'portfolio_size_after': len(active_investments) + 1,
            'category_concentration_percent': category_concentration.quantize(Decimal('0.1'), rounding=ROUND_HALF_UP),
            'concentration_risk': concentration_risk,
            'diversification_benefit': diversification_benefit,
            'category_distribution': category_counts,
            'fit_score': 100 - (category_concentration - 25) if category_concentration > 25 else 100
        }

    def _generate_investment_recommendation(self, financial_analysis: Dict[str, Any],
        ...
        if composite_score >= 80:
            ...
            decision = "RECOMMEND"
            rationale = "Good investment opportunity with acceptable risk-return profile"
        elif composite_score >= 50:
            decision = "CONDITIONAL APPROVE"
            rationale = "Moderate opportunity - recommend with risk mitigation measures"
        else:
            decision = "NOT RECOMMENDED"
            rationale = "Investment does not meet minimum criteria for risk-adjusted returns"

        return {
            'decision': decision,
            'composite_score': round(composite_score, 1),
            'rationale': rationale,
            'key_strengths': self._identify_key_strengths(financial_analysis, risk_assessment, strategic_score),
            'key_concerns': self._identify_key_concerns(financial_analysis, risk_assessment),
            'conditions': self._generate_approval_conditions(risk_assessment, portfolio_analysis)
        }

    def _identify_key_strengths(self, financial: Dict[str, Any], risk: Dict[str, Any], strategic: Decimal) -> List[str]:
        ...
        if financial['payback_period_months'] <= 18:
            strengths.append(f"Fast payback period of {financial['payback_period_months']} months")
    ...
    def _identify_key_concerns(self, financial: Dict[str, Any], risk: Dict[str, Any]) -> List[str]:
    ...
    def _generate_approval_conditions(self, risk: Dict[str, Any], portfolio: Dict[str, Any]) -> List[str]:
        ...
        if portfolio['concentration_risk']:
            conditions.append("Monitor portfolio concentration and consider diversification")
        conditions.append("Establish regular progress reviews and success metric tracking")

        return conditions

    # Additional helper methods for tracking and analysis

    def _measure_actual_benefits(self, proposal: InvestmentProposal, period: str) -> Decimal:
        ...
        return proposal.year_1_benefits * Decimal('0.85')  # Placeholder: 85% of projected

    def _get_planned_benefits_for_period(self, proposal: InvestmentProposal, period: str) -> Decimal:
    ...
    def _measure_stakeholder_satisfaction(self, proposal: InvestmentProposal) -> Decimal:
        """Measure stakeholder satisfaction"""
        # Placeholder - would survey affected stakeholders
        return Decimal('8.2')  # 8.2/10 satisfaction

    def _breakdown_benefits_by_type(self, total_benefits: Decimal, proposal: InvestmentProposal) -> Dict[str, Decimal]:
        """Break down benefits by type"""
    ...
    def _capture_lessons_learned(self, proposal: InvestmentProposal) -> Dict[str, List[str]]:
    ...
    def _calculate_total_benefits_realized(self, investments: List[InvestmentProposal], period: str) -> Decimal:
    ...
    def _analyze_portfolio_performance(self, investments: List[InvestmentProposal]) -> Dict[str, Any]:
    ...
    def _analyze_category_performance(self, investments: List[InvestmentProposal]) -> Dict[str, Any]:
        ...
        for inv in investments:
            ...
            projected_roi = ((inv.year_1_benefits + inv.year_2_benefits + inv.year_3_benefits) / inv.total_investment) * 100
            category_roi[category] = (category_roi[category] + projected_roi) / 2  # Average

        return {
            'investment_amounts': {k: str(v) for k, v in category_investments.items()},
            'roi_by_category': {k: str(v.quantize(Decimal('0.1'), rounding=ROUND_HALF_UP)) for k, v in category_roi.items()}
        }

    def _analyze_portfolio_risks(self, investments: List[InvestmentProposal]) -> Dict[str, Any]:
        ...
        for inv in investments:
            ...
            if inv.total_investment > Decimal('200000') or inv.estimated_duration_months > 12:
                high_risk_count += 1
    ...
    def _generate_portfolio_recommendations(self, investments: List[InvestmentProposal],
    ...
    def _generate_executive_summary(self, proposal: InvestmentProposal, evaluation: Dict[str, Any]) -> str:
...
Strategic Value: This investment aligns with our platform modernization strategy and addresses critical business needs while delivering strong financial returns."""
    ...
    def _create_detailed_financial_model(self, proposal: InvestmentProposal) -> Dict[str, Any]:
        """Create detailed financial model"""
        return {
    ...
    def _create_implementation_roadmap(self, proposal: InvestmentProposal) -> Dict[str, Any]:
    ...
    def _define_success_measurement_framework(self, proposal: InvestmentProposal) -> Dict[str, Any]:
        """Define success measurement framework"""
        return {
    ...
    def _get_detailed_calculations(self, proposal: InvestmentProposal) -> Dict[str, Any]:
    ...
    def _get_benchmarking_data(self, category: InvestmentCategory) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': '72%',
            'common_risk_factors': ['integration complexity', 'adoption challenges', 'scope creep']
        }

    def _analyze_stakeholder_impact(self, proposal: InvestmentProposal) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_dependencies': ['stakeholder engagement', 'change adoption', 'technical execution']
        }
