...
class ExecutiveDashboardGenerator:
    ...
    def generate_dashboard(
        self, view: DashboardView, executive_role: str = "Director of Engineering"
    ) -> ExecutiveDashboard:
        """
        ...
        try:
            ...
            business_impact = (
                self.business_calculator.calculate_comprehensive_business_impact(30)
            )
            dashboard_metrics = (
            ...
            if view == DashboardView.STRATEGIC_OVERVIEW:
                metrics = self._generate_strategic_overview_metrics(
                    business_impact, dashboard_metrics
                )
            elif view == DashboardView.FINANCIAL_PERFORMANCE:
                metrics = self._generate_financial_performance_metrics(
                    business_impact, portfolio_summary
                )
            elif view == DashboardView.OPERATIONAL_EXCELLENCE:
                metrics = self._generate_operational_excellence_metrics(
                    business_impact, dashboard_metrics
                )
            elif view == DashboardView.STAKEHOLDER_HEALTH:
                metrics = self._generate_stakeholder_health_metrics(
                    business_impact, dashboard_metrics
                )
            elif view == DashboardView.RISK_INTELLIGENCE:
                metrics = self._generate_risk_intelligence_metrics(
                    business_impact, portfolio_summary
                )
            else:  # COMPETITIVE_POSITION
                metrics = self._generate_competitive_position_metrics(
                    business_impact, dashboard_metrics
                )

            # Generate alerts
            alerts = self._generate_dashboard_alerts(
                business_impact, portfolio_summary, dashboard_metrics
            )

            # Generate strategic insights
            insights = self._generate_strategic_insights(
                business_impact, dashboard_metrics, view
            )
            ...
            achievements = self._extract_key_achievements(
                business_impact, portfolio_summary
            )
            ...
            self.logger.info(
                ...
                view=view.value,
                metrics_count=len(metrics["headline_metrics"])
                + len(metrics["performance_metrics"]),
            ...
            self.logger.error(
                "Dashboard generation failed", view=view.value, error=str(e)
            )
        ...
        try:
            ...
            kpi_summary = {
                ...
                "critical_alerts": len(
                    [
                        a
                        for a in self._generate_dashboard_alerts(
                            business_impact, portfolio_summary, {}
                        )
    ...
    def export_dashboard_data(
        self, dashboard: ExecutiveDashboard, format: str = "json"
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_strategic_overview_metrics(
        ...
        headline_metrics = [
            DashboardMetric(
                ...
                status=self._determine_metric_status(
                    "business_value", business_impact.total_business_value
                ),
                chart_type="number",
    ...
    def _generate_financial_performance_metrics(
    ...
    def _generate_operational_excellence_metrics(
        self, business_impact, dashboard_metrics
    ) -> Dict[str, List[DashboardMetric]]:
    ...
    def _generate_stakeholder_health_metrics(
        self, business_impact, dashboard_metrics
    ) -> Dict[str, List[DashboardMetric]]:
    ...
    def _generate_risk_intelligence_metrics(
        self, business_impact, portfolio_summary
    ) -> Dict[str, List[DashboardMetric]]:
    ...
    def _generate_competitive_position_metrics(
        self, business_impact, dashboard_metrics
    ) -> Dict[str, List[DashboardMetric]]:
    ...
    def _generate_dashboard_alerts(
        self, business_impact, portfolio_summary, dashboard_metrics
    ) -> List[DashboardAlert]:
    ...
    def _generate_strategic_insights(
        self, business_impact, dashboard_metrics, view: DashboardView
    ...
    def _calculate_overall_health_score(
        self, headline_metrics: List[DashboardMetric]
    ) -> Decimal:
        """Calculate overall health score from headline metrics"""
        if not headline_metrics:
            return Decimal("75")  # Default

        status_scores = {"excellent": 100, "good": 80, "warning": 60, "critical": 30}

        total_score = sum(
            status_scores.get(metric.status, 60) for metric in headline_metrics
        )
    ...
    def _generate_priority_actions(
        self, alerts: List[DashboardAlert], insights: List[DashboardInsight]
    ) -> List[str]:
    ...
    def _create_error_dashboard(
        self, view: DashboardView, executive_role: str, error_message: str
    ) -> ExecutiveDashboard:
