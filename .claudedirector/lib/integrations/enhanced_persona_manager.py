...
from .mcp_use_client import MCPUseClient
...
class TransparencyManager:
    ...
    def get_enhanced_message(self, persona: str, server: str, processing_time: float) -> str:
...
class EnhancedPersonaManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.persona_configs = {
            ...
            "rachel": {
                "primary_server": "context7",
                "capabilities": ["design_system_methodology", "cross_team_coordination"],
                "enhancement_threshold": 0.6,
                "timeout": 8
            },
            "alvaro": {
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if connection_status.success_rate > 0:
                logger.info(f"Enhanced persona manager initialized: {connection_status.available_servers} servers available")
                self.is_initialized = True
                return True
            else:
    ...
    async def get_enhanced_response(self, persona: str, user_input: str, context: Optional[Dict[str, Any]] = None) -> EnhancedResponse:
        """
        ...
        complexity_analysis = self.complexity_analyzer.analyze_complexity(user_input, persona)

        # Check if enhancement should be attempted
        if not self._should_enhance(persona, complexity_analysis):
            # Return standard response
            standard_response = await self._get_standard_response(persona, user_input, context)
        ...
        return await self._get_enhanced_response(persona, user_input, complexity_analysis, context, start_time)

    def _should_enhance(self, persona: str, complexity_analysis: ComplexityAnalysis) -> bool:
        """
        ...
        if complexity_analysis.recommended_enhancement:
            ...
            enhancement_mapping = {
                "systematic_analysis": ["systematic_analysis", "organizational_scaling"],
                "architecture_patterns": ["architecture_patterns", "technical_frameworks"],
                "design_system_methodology": ["design_system_methodology", "cross_team_coordination"],
                "business_strategy": ["business_strategy", "competitive_analysis"],
                "technology_leadership": ["technology_leadership", "organizational_scaling"]
            }
    ...
    async def _get_enhanced_response(self, persona: str, user_input: str, complexity_analysis: ComplexityAnalysis,
                                   context: Optional[Dict[str, Any]], start_time: float) -> EnhancedResponse:
        """
        ...
        try:
            ...
            if not self.mcp_client.is_server_available(primary_server):
                return await self._handle_fallback(persona, user_input, context, start_time,
                                                 f"Server {primary_server} not available")

            # Prepare analysis query
            analysis_query = self._prepare_analysis_query(persona, user_input, complexity_analysis, context)

            # Execute MCP analysis
            mcp_response = await self.mcp_client.execute_analysis(primary_server, analysis_query, timeout)
            ...
            enhanced_content = await self._blend_response(persona, user_input, mcp_response.content, context)
            ...
            return await self._handle_fallback(persona, user_input, context, start_time, str(e))

    async def _handle_fallback(self, persona: str, user_input: str, context: Optional[Dict[str, Any]],
                             start_time: float, reason: str) -> EnhancedResponse:
        """
    ...
    def _prepare_analysis_query(self, persona: str, user_input: str, complexity_analysis: ComplexityAnalysis,
        ...
        if persona == "diego":
            ...
            if "architecture" in user_input.lower() or "technical" in user_input.lower():
                query_prefix = "Find relevant architectural patterns and technical frameworks for:"
            else:
            ...
            if "design" in user_input.lower() or "system" in user_input.lower():
                ...
                query_prefix = "Analyze this design challenge using scaling methodologies:"
        elif persona == "alvaro":
            if "business" in user_input.lower() or "strategy" in user_input.lower():
                ...
                query_prefix = "Apply financial modeling and ROI analysis frameworks to:"
            else:
                query_prefix = "Analyze this business challenge using strategic frameworks:"
        elif persona == "camille":
            if "technology" in user_input.lower() and "leadership" in user_input.lower():
        ...
        persona_context = ""
        if persona == "martin":
            persona_context = " [Focus: Technical architecture, scalability patterns, system design]"
        elif persona == "rachel":
            persona_context = " [Focus: Design systems, cross-team coordination, UX methodology]"
        elif persona == "alvaro":
            persona_context = " [Focus: Business strategy, competitive analysis, financial modeling]"

        return f"{query_prefix} {user_input}{triggers_context}{persona_context}"

    async def _blend_response(self, persona: str, user_input: str, mcp_content: str,
                            context: Optional[Dict[str, Any]]) -> str:
        """
        Blend MCP response with persona characteristics
        ...
        if mcp_content and mcp_content.strip():
            ...
            if any(pattern in content_lower for pattern in ["pattern", "architecture", "design"]):
                intro = persona_style['pattern_intro']
            elif any(framework in content_lower for framework in ["framework", "methodology", "approach"]):
            ...
            if persona == "martin" and len(clean_content) > 100:
                blended_response += " This approach provides a solid technical foundation."
            elif persona == "rachel" and len(clean_content) > 100:
                blended_response += " This ensures we can scale effectively across teams."
    ...
    async def _get_standard_response(self, persona: str, user_input: str,
                                   context: Optional[Dict[str, Any]]) -> str:
        """
        ...
        if persona == "diego":
            if any(keyword in user_input.lower() for keyword in ["team", "organization", "scaling", "structure"]):
            ...
            if any(keyword in user_input.lower() for keyword in ["architecture", "technical", "system", "design"]):
            ...
            if any(keyword in user_input.lower() for keyword in ["design", "system", "user", "experience", "team"]):
            ...
            if any(keyword in user_input.lower() for keyword in ["business", "strategy", "market", "competitive"]):
            ...
            elif any(keyword in user_input.lower() for keyword in ["roi", "financial", "cost", "investment"]):
            ...
            if any(keyword in user_input.lower() for keyword in ["technology", "leadership", "organizational"]):
    ...
    def get_server_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "enhanced_personas": [p for p in self.persona_configs.keys()
