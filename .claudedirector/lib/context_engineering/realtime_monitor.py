...
class RealTimeMonitor(BaseProcessor):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None, **kwargs):
        ...
        self.detection_latency_target = self.get_config("detection_latency_seconds", 300)  # 5 minutes
        self.performance_monitoring_enabled = self.get_config("performance_monitoring", True)
        ...
        if self.events_processed > 0:
            self.average_detection_latency = (
                (self.average_detection_latency * (self.events_processed - 1) 
                 + processing_time) / self.events_processed
            )
        else:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        sla_compliance_rate = (
            ((self.events_processed - self.latency_violations) / self.events_processed * 100)
            if self.events_processed > 0 else 100.0
        )
