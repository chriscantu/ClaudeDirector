...
class IntelligentTaskDetector:
    ...
    def detect_tasks_in_content(self, content: str, context: Dict) -> List[Dict]:
        ...
        try:
            ...
            for task_text, assignment_direction in all_tasks:
                analysis = self._analyze_task_candidate(
                    task_text, assignment_direction, normalized_content, context
                )
            ...
            self.logger.info(
                ...
                high_confidence=len(
                    [
                        ...
                        if t["confidence_score"] >= self.AUTO_CREATE_THRESHOLD
                    ]
                ),
            )
        ...
        for pattern in self.assignment_patterns["outgoing_from_me"]:
            ...
            for match in matches:
                if len(match.groups()) >= 2:
                    ...
                    if self._is_valid_task(task_text) and self._is_valid_assignee(
                        assignee
                    ):
    ...
    def _is_valid_task(self, task_text: str) -> bool:
        ...
        has_action = any(
            verb in task_lower for verb in self.task_patterns["action_verbs"]
        )
        if not has_action:
            return False
    ...
    def _analyze_task_candidate(
        ...
        analysis["follow_up_required"] = self._requires_follow_up(
            task_text, assignment_direction
        )

        # Calculate confidence score
        analysis["confidence_score"] = self._calculate_task_confidence(
            analysis, full_content, context
        )
    ...
    def _analyze_priority(self, task_text: str, full_content: str) -> Dict:
        ...
        return {"level": best_priority, "score": best_score}
    ...
    def _analyze_category_and_scope(self, task_text: str, context: Dict) -> Dict:
        ...
        if any(
            word in task_lower
            for word in ["platform", "architecture", "system", "infrastructure"]
        ):
            category = "platform_initiative"
        elif any(
            word in task_lower
            for word in ["follow up", "check in", "circle back", "touch base"]
        ):
            category = "stakeholder_followup"
        elif any(
            word in task_lower
            for word in ["strategic", "roadmap", "vision", "planning"]
        ):
        ...
        if any(
            word in task_lower
            for word in ["platform", "all teams", "organization", "company"]
        ):
            scope = "platform_wide"
        elif any(
            word in task_lower
            for word in ["cross-team", "multiple teams", "coordination"]
        ):
            scope = "cross_team"
    ...
    def _requires_follow_up(self, task_text: str, assignment_direction: str) -> bool:
        ...
        if any(
            word in task_lower
            for word in ["platform", "strategic", "critical", "important"]
        ):
            return True
    ...
    def _extract_task_context(
        self, task_text: str, full_content: str, window: int = 100
    ) -> str:
    ...
    def _calculate_task_confidence(
        self, analysis: Dict, full_content: str, context: Dict
    ) -> float:
        ...
        try:
            ...
            for i, task in enumerate(tasks, 1):
                confidence_emoji = (
                    ...
                    else "🟡" if task["confidence_score"] >= 0.6 else "🔴"
                )
                direction_emoji = {
                    "incoming": "📥",
                    "outgoing": "📤",
                ...
                print(
                    f"{confidence_emoji} {direction_emoji} Task {i}: {task['task_text']}"
                )
