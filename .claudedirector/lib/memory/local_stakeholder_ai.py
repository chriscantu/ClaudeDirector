...
class LocalStakeholderAI:
    ...
    def detect_stakeholders_in_content(self, content: str, context: Dict) -> List[Dict]:
        ...
        try:
            ...
            for name in potential_names:
                analysis = self._analyze_stakeholder_candidate(
                    name, normalized_content, context
                )
            ...
            stakeholder_candidates.sort(
                key=lambda x: x["confidence_score"], reverse=True
            )

            self.logger.info(
                "Detected stakeholder candidates",
                count=len(stakeholder_candidates),
                high_confidence=len(
                    [
                        s
                        for s in stakeholder_candidates
                        if s["confidence_score"] >= self.AUTO_CREATE_THRESHOLD
                    ]
                ),
            )
        ...
        for email_pattern in self.name_patterns["email_patterns"]:
            ...
            for email in email_matches:
                ...
                if "." in name_part:
                    ...
                    if len(name_parts) == 2:
                        potential_name = (
                            f"{name_parts[0].title()} {name_parts[1].title()}"
                        )
    ...
    def _analyze_stakeholder_candidate(
        self, name: str, content: str, context: Dict
    ) -> Dict:
        ...
        importance_analysis = self._analyze_importance_indicators(
    ...
    def _analyze_role_indicators(self, name: str, content: str) -> Dict:
        ...
        max_possible_score = max(
            role_data["weight"] for role_data in self.role_patterns.values()
        )
        confidence = (
            min(1.0, best_score / max_possible_score) if best_score > 0 else 0.0
        )

        return {"role": best_role, "confidence": confidence, "raw_score": best_score}

    def _analyze_importance_indicators(
        self, name: str, content: str, context: Dict
    ) -> Dict:
        ...
        return {"level": level, "score": total_score}
    ...
    def _analyze_communication_patterns(self, name: str, content: str) -> Dict:
        ...
        preferences = {"channels": [], "style": None, "frequency_indicators": []}

        # Detect preferred channels
        for channel, indicators in self.communication_patterns[
            "channel_indicators"
        ].items():
        ...
        style_scores = {}
        for style, indicators in self.communication_patterns[
            "style_indicators"
        ].items():
            score = sum(
                1
    ...
    def _calculate_confidence_score(self, analysis: Dict) -> float:
        ...
        if analysis["source_context"].get("category") == "meeting_prep":
            context_score += 0.5
        if analysis["source_context"].get("meeting_type") in [
            "vp_1on1",
            "strategic_planning",
        ]:
            context_score += 0.5
    ...
    def check_existing_stakeholder(self, stakeholder_key: str) -> Optional[Dict]:
        ...
        try:
            ...
            self.logger.error(
                "Failed to check existing stakeholder",
                stakeholder_key=stakeholder_key,
                error=str(e),
            )
            return None

    def suggest_stakeholder_updates(
        self, stakeholder_key: str, new_analysis: Dict
    ) -> List[Dict]:
        ...
        if (
            new_analysis.get("detected_role")
            and new_analysis["role_confidence"] > 0.7
            and new_analysis["detected_role"] != existing.get("role_title")
        ):
        ...
        if (
            new_analysis.get("strategic_importance")
            != existing.get("strategic_importance")
            and new_analysis.get("importance_score", 0) > 5
        ):
        ...
        new_channels = new_analysis.get("communication_preferences", {}).get(
            "channels", []
        )
    ...
    if args.analyze_content:
        ...
        for candidate in candidates:
            confidence_emoji = (
                ...
                else "🟡" if candidate["confidence_score"] >= 0.6 else "🔴"
            )

            print(
                f"{confidence_emoji} {candidate['name']} ({candidate['stakeholder_key']})"
            )
            print(
                f"   Role: {candidate['detected_role']} (confidence: {candidate['role_confidence']:.1%})"
            )
            print(
                f"   Importance: {candidate['strategic_importance']} (score: {candidate['importance_score']:.1f})"
            )
            print(f"   Overall Confidence: {candidate['confidence_score']:.1%}")

            if candidate["communication_preferences"]["channels"]:
                print(
                    f"   Preferred Channels: {', '.join(candidate['communication_preferences']['channels'])}"
                )

            if candidate["communication_preferences"]["style"]:
                print(
                    f"   Communication Style: {candidate['communication_preferences']['style']}"
                )
