...
class SessionContextManager(StrategicMemoryManager):
    ...
    def _get_active_personas(self) -> List[str]:
                    ...
                    if row[0]:
                        ...
                        if isinstance(conversation_data, list):
                            for turn in conversation_data:
                                if isinstance(turn, dict) and turn.get('personas_activated'):
        ...
        persona_patterns = {
            ...
            'martin': ['platform architecture', 'technical architecture', 'evolutionary']
        }
    ...
    def _get_roi_discussion_context(self) -> Dict[str, Any]:
        ...
        roi_keywords = ['roi', 'return on investment', 'cost', 'budget', 'investment', 'value', 'savings']
        platform_investment_keywords = ['platform investment', 'infrastructure cost', 'developer productivity']
                        ...
                        if isinstance(conversation_data, list):
                            for turn in conversation_data:
                                if isinstance(turn, dict):
                                    ...
                                    if any(keyword in text for keyword in platform_investment_keywords):
                                        roi_context['investment_decisions'].append({
                                            'timestamp': row[1],
                                            'topic': 'platform_investment',
                                            'discussion_snippet': text[:150] + '...' if len(text) > 150 else text
                                        })

                                    # Extract specific metrics mentioned
                                    if 'efficiency' in text or 'productivity' in text or 'cost savings' in text:
                                        roi_context['key_metrics'].append({
                                            'timestamp': row[1],
                                            'metric_type': 'efficiency_productivity',
                                            'mention': text[:100] + '...' if len(text) > 100 else text
                                        })
        ...
        try:
            ...
            for initiative in initiatives:
                if any(keyword in initiative.get('description', '').lower()
        ...
        try:
            with self.get_connection() as conn:
                ...
                for stakeholder in stakeholders:
                    stakeholder_key, name, role, style, context, last_interaction = stakeholder
                    ...
                    platform_positive = any(phrase in full_context for phrase in [
                        'platform investment', 'platform advocate', 'supports platform',
                        'infrastructure benefits', 'technical excellence', 'developer productivity'
                    ])

                    # Platform opposition indicators
                    platform_negative = any(phrase in full_context for phrase in [
                        'product delivery focus', 'opposes platform', 'product-first',
                        'platform skeptical', 'delivery over platform', 'anti-platform'
                    ])

                    # Executive/influence indicators
                    high_influence = any(phrase in role.lower() for phrase in [
                        'vp', 'director', 'executive', 'chief', 'head of'
                    ]) if role else False
                    ...
                    if platform_positive:
                        ...
                        coalition_context['neutral_stakeholders'].append(stakeholder_data)
                                ...
                                for turn in conversation_data:
                                    if isinstance(turn, dict):
                                        ...
                                        if any(phrase in text for phrase in [
                                            'stakeholder relationship', 'coalition', 'alliance',
                                            'opposition', 'support', 'resistance'
                                        ]):
                                            coalition_context['key_relationships'].append({
                                                'timestamp': row[1],
                                                'relationship_context': text[:200] + '...' if len(text) > 200 else text
                                            })

                                        # Detect influence patterns
                                        if any(phrase in text for phrase in [
                                            'executive influence', 'decision maker', 'key stakeholder',
                                            'leadership team', 'senior level'
                                        ]):
                                            coalition_context['influence_patterns'].append({
                                                'timestamp': row[1],
                                                'pattern': text[:150] + '...' if len(text) > 150 else text
                                            })
