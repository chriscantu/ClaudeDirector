...
class EnhancedFrameworkRecommendation:
    ...
    def meets_phase93_criteria(self) -> bool:
        ...
        return (
            self.confidence_score >= FrameworkConfidenceLevel.HIGH.value
            and self.quality_score >= AnalysisQualityLevel.GOOD.value
        )
...
class StrategicAnalysisResult:
    ...
    def meets_executive_standards(self) -> bool:
        ...
        return (
            ...
            and self.decision_support_accuracy >= 0.9  # 90%+ requirement
            and len(self.executive_summary) > 0
        )
...
class EnhancedFrameworkEngine(BaseProcessor):
    ...
    async def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if analysis_type == "framework_detection":
                result = await self._process_framework_detection(
                    content, executive_context
                )
            elif analysis_type == "strategic_analysis":
                result = await self._process_strategic_analysis(
                    content, executive_context
                )
            elif analysis_type == "multi_framework":
                result = await self._process_multi_framework_analysis(
                    content, executive_context
                )
            else:
                result = await self._process_comprehensive_analysis(
                    content, executive_context
                )
    ...
    async def _process_strategic_analysis(
        ...
        return {
            ...
            "quality_metrics": {
                ...
                "meets_decision_threshold": decision_support_accuracy
                >= self.decision_support_target,
            },
        }

    # === ENHANCED DETECTION METHODS ===

    async def _detect_frameworks_enhanced(
        self, content: str
    ) -> List[EnhancedFrameworkRecommendation]:
        ...
        all_matches = self._combine_detection_results(
            pattern_matches, semantic_matches, context_matches
        )
    ...
    async def _calculate_enhanced_confidence(
        self, match: Dict[str, Any], content: str
    ) -> float:
        ...
        pattern_strength = len(match.get("matched_patterns", [])) / max(
            len(match.get("total_patterns", [])), 1
        )

        # Factor 2: Context relevance
        context_relevance = await self._calculate_context_relevance(
            match["framework_name"], content
        )

        # Factor 3: Historical accuracy for this framework
        historical_accuracy = await self._get_framework_historical_accuracy(
            match["framework_name"]
        )

        # Factor 4: Content complexity bonus
        complexity_bonus = min(
            len(content.split()) / 100, 0.1
        )  # Up to 10% bonus for complex content
    ...
    async def _calculate_decision_support_accuracy(
        self, analysis: str, content: str
    ) -> float:
        """Calculate decision support accuracy (target 90%+)"""
    ...
    def _initialize_framework_processor(self) -> Optional[FrameworkProcessor]:
        ...
        try:
            ...
            self.logger.warning(
                "Framework processor initialization failed", error=str(e)
            )
            return None
    ...
    def _initialize_multi_framework_detector(self) -> Dict[str, Any]:
        ...
        return {
            "combination_patterns": {
                "Team Topologies + Good Strategy Bad Strategy": [
                    "organizational",
                    "strategy",
                    "teams",
                ],
                "WRAP Framework + Capital Allocation": [
                    "decision",
                    "investment",
                    "resource",
                ],
                "Crucial Conversations + Scaling Up Excellence": [
                    "communication",
                    "growth",
                    "leadership",
                ],
            },
            "complexity_thresholds": {"simple": 1, "moderate": 2, "complex": 3},
        }
        ...
        for framework_name, config in framework_patterns.items():
            ...
            if len(concept_matches) >= config.min_semantic_matches:
                semantic_confidence = min(
                    (len(concept_matches) / len(config.semantic_concepts))
                    * config.weight,
                    1.0,
                )
    ...
    def _combine_detection_results(self, *results) -> List[Dict[str, Any]]:
        ...
        for result_set in results:
            for match in result_set:
                ...
                combined_frameworks[framework_name]["confidence_scores"].append(
                    match["confidence"]
                )
                combined_frameworks[framework_name]["reasoning_sources"].extend(
                    match.get("reasoning", [])
                )

                # Track detection method
                if "matched_patterns" in match:
                    combined_frameworks[framework_name]["detection_methods"].append(
                        "pattern"
                    )
        ...
        for framework_name, data in combined_frameworks.items():
            ...
            base_confidence = sum(data["confidence_scores"]) / len(
                data["confidence_scores"]
            )

            # Bonus for multiple detection methods (up to 15% boost)
            method_diversity_bonus = min(
                len(set(data["detection_methods"])) * 0.05, 0.15
            )
            ...
            final_confidence = min(
                base_confidence + method_diversity_bonus + evidence_bonus, 1.0
            )
            ...
            final_results.append(
                {
                    ...
                    + data["reasoning_sources"][
                        :3
                    ],  # Include top 3 specific reasoning points
                }
            )

        return sorted(final_results, key=lambda x: x["confidence"], reverse=True)

    async def _analyze_multi_framework_context(
        self, content: str, frameworks: List
    ) -> Dict[str, Any]:
    ...
    async def _generate_strategic_analysis(self, content: str) -> str:
        ...
        analysis_sections.append(
            f"EXECUTIVE SUMMARY: {strategic_elements.get('summary', 'Strategic analysis required')}"
        )

        # Strategic Context
        if strategic_elements.get("context"):
            analysis_sections.append(
                f"STRATEGIC CONTEXT: {strategic_elements['context']}"
            )

        # Framework Recommendations
        recommended_frameworks = await self._get_framework_recommendations_for_analysis(
            content
        )
        if recommended_frameworks:
            framework_text = ", ".join(
                [fw.framework_name for fw in recommended_frameworks[:3]]
            )
            analysis_sections.append(f"RECOMMENDED FRAMEWORKS: {framework_text}")

        # Strategic Options
        options = strategic_elements.get(
            "options",
            [
                "Option A: Maintain current approach",
                "Option B: Implement strategic changes",
            ],
        )
        analysis_sections.append(f"STRATEGIC OPTIONS: {'; '.join(options)}")

        # Risk Assessment
        risks = strategic_elements.get(
            "risks",
            [
        ...
        analysis_sections.append(
            "IMPLEMENTATION ROADMAP: Phase 1 - Assessment and Planning; Phase 2 - Execution and Monitoring; Phase 3 - Optimization and Scaling"
        )

        # Success Metrics
        metrics = strategic_elements.get(
            "metrics",
            [
    ...
    async def _generate_executive_summary(
        self, analysis: str, executive_context: bool
    ) -> str:
        """Generate executive summary"""
        return "Executive Summary: Strategic recommendation with high confidence."

    async def _get_supporting_frameworks(
        self, content: str
    ) -> List[EnhancedFrameworkRecommendation]:
    ...
    async def _get_framework_recommendations_for_analysis(
        self, content: str
    ) -> List[EnhancedFrameworkRecommendation]:
    ...
    async def _calculate_context_relevance(
        self, framework_name: str, content: str
    ) -> float:
    ...
    async def _process_multi_framework_analysis(
        self, content: str, executive_context: bool
    ) -> Dict[str, Any]:
    ...
    async def _calculate_framework_quality_score(
        self, match: Dict[str, Any], content: str
    ) -> float:
...
def create_enhanced_framework_engine(
