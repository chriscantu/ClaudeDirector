...
class StrategicAnalyzer:
    ...
    def calculate_strategic_impact(self, issue: JiraIssue) -> StrategicScore:
        ...
        # Platform/DevEx indicators (SECURITY: Generic patterns only, no company-specific tool names)
        platform_patterns = r"(platform|developer.experience|DevEx|tooling|automation|webpack|vite|build.tool|bundler|compiler)"
    ...
    def _detect_initiative_level(self, summary: str) -> str:
        ...
        if "L0:" in summary or "Layer 0" in summary:
            ...
            # Infer from content (SECURITY: Generic keywords only, no company-specific initiative names)
            compliance_keywords = [
                "compliance",
                "security",
                "audit",
...
class JiraReporter:
    ...
    def __init__(self, config: ConfigManager, duration_days: int = 7):
        ...
        logger.info(
            f"JiraReporter initialized: {self.duration_label} report ({self.duration_days} days)"
        )
