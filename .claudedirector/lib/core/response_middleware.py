...
class ResponseTransparencyMiddleware:
    ...
    def process_response(self, response: str, user_input: str = "", metadata: Optional[Dict] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            transparency_summary = get_transparency_summary(enhanced_response, user_input)

            # Calculate processing time
            processing_time = time.time() - start_time

            # Update performance stats
            self._update_stats(processing_time, transparency_summary.get('transparency_applied', False))
...
def get_transparency_middleware(config: Optional[Dict] = None) -> ResponseTransparencyMiddleware:
...
def apply_response_transparency(response: str, user_input: str = "", metadata: Optional[Dict] = None) -> str:
    """
...
def get_response_transparency_summary(response: str, user_input: str = "") -> Dict[str, Any]:
    """
...
def enhance_conversation_turn(user_input: str, assistant_response: str, personas_activated: Optional[List[str]] = None, metadata: Optional[Dict] = None) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        print(f"   Header: {'✅' if result['transparency_summary'].get('has_persona_header') else '❌'}")
        print(f"   Time: {result['processing_time']*1000:.2f}ms")
        print(f"   Applied: {'✅' if result['transparency_summary'].get('transparency_applied') else '❌'}")

    # Performance stats
    stats = middleware.get_performance_stats()
    print(f"\n📊 Performance Stats:")
    print(f"   Total responses: {stats['total_responses']}")
    print(f"   Enhanced: {stats['enhanced_responses']} ({stats['enhancement_rate']:.1f}%)")
