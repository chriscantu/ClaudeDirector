    ...
    def get_transparency_disclosure(self, persona: str = "martin") -> str:
        ...
        for server_name, calls in server_calls.items():
            ...
            if capabilities:
                ...
                server_time = sum(
                    call.processing_time for call in calls if call.success
                )
                disclosure_lines.append(
                    f"• {server_name.title()} Server: {capability_text} ({server_time:.2f}s)"
                )

        # Add failure notice if needed
        if failed_calls:
            disclosure_lines.append(
                "• Note: Some enhanced features temporarily unavailable"
            )

        disclosure_lines.append("---")
        disclosure_lines.append(
            f"**Enhanced Intelligence**: Analysis powered by strategic frameworks and collaborative AI systems."
        )
    ...
    def get_mcp_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_processing_time": sum(
                call.processing_time for call in self.active_calls
            ),
            "successful_calls": len(
                [call for call in self.active_calls if call.success]
            ),
            "failed_calls": len(
                [call for call in self.active_calls if not call.success]
            ),
            "capabilities_used": list(
                set(call.capability for call in self.active_calls if call.success)
            ),
        }
...
class MCPTransparencyMiddleware:
    ...
    def wrap_response_with_mcp_disclosure(
        self, response: str, persona: str = "martin"
    ) -> str:
    ...
    def _auto_detect_mcp_usage(self, response: str, user_input: str):
        ...
        if any(
            ...
            for word in ["strategic", "analysis", "framework", "systematic"]
        ):
        ...
        if any(
            word in response_lower
            for word in ["architecture", "pattern", "design", "component"]
        ):
        ...
        if any(
            word in response_lower
            for word in ["roi", "investment", "business value", "competitive"]
        ):
        ...
        if any(
            word in response_lower
            for word in ["diagram", "visual", "presentation", "chart"]
        ):
...
def apply_mcp_transparency_to_response(
    response: str, user_input: str = "", mcp_calls: Optional[List[Dict]] = None
) -> str:
...
if __name__ == "__main__":
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        print(
            f"   Processing time: {result['mcp_summary']['total_processing_time']:.2f}s"
        )
        print(
            f"   Transparency applied: {'✅' if result['transparency_applied'] else '❌'}"
        )
    ...
    print(
        f"   Available servers: {list(middleware.tracker.server_capabilities.keys())}"
    )
