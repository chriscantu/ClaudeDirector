...
try:
    ...
    logger.warning("Hybrid database types not available, bridge may have limited compatibility")
...
class HybridToUnifiedBridge:
    ...
    def __init__(self):
        ...
        if UNIFIED_DB_AVAILABLE:
            try:
                ...
                self.logger.info("✅ HybridToUnifiedBridge initialized with UnifiedDatabaseCoordinator")
            except Exception as e:
                self.unified_coordinator = None
                self.logger.error(f"Failed to initialize UnifiedDatabaseCoordinator: {e}")
        else:
            self.unified_coordinator = None
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if conn:
                self.logger.info("✅ HybridToUnifiedBridge connected via UnifiedDatabaseCoordinator")
                return True
        except Exception as e:
    ...
    def execute_query(self, query: str, context: Optional['QueryContext'] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.debug(f"Bridge query executed successfully in {execution_time_ms:.2f}ms")
            return hybrid_result
            
        except Exception as e:
            execution_time_ms = (time.time() - start_time) * 1000
            self._update_metrics(execution_time_ms, success=False)
            self.logger.error(f"Bridge query failed after {execution_time_ms:.2f}ms: {e}")
    ...
    def _map_query_context(self, hybrid_context: 'QueryContext') -> Dict[str, Any]:
        ...
        try:
            unified_context = {
                "query_type": hybrid_context.query_type.value if hasattr(hybrid_context.query_type, 'value') else str(hybrid_context.query_type),
                "workload_pattern": hybrid_context.workload_pattern.value if hasattr(hybrid_context.workload_pattern, 'value') else str(hybrid_context.workload_pattern),
                "priority": getattr(hybrid_context, 'priority', 'normal'),
                "cache_enabled": getattr(hybrid_context, 'cache_enabled', True),
                "expected_result_size": getattr(hybrid_context, 'expected_result_size', 0),
                "estimated_execution_time_ms": getattr(hybrid_context, 'estimated_execution_time_ms', 0),
            }
    ...
    def _map_result_to_hybrid_format(self, unified_result: Any, execution_time_ms: float) -> Dict[str, Any]:
            ...
            if hasattr(unified_result, 'success'):
                return {
                    ...
                    "performance_metrics": {
                        ...
                        "strategy_used": getattr(unified_result, 'strategy_used', 'sqlite'),
                    },
                    "error": getattr(unified_result, 'error_message', None) if not unified_result.success else None,
                }
            
            # Handle direct query results (list/dict)
            elif isinstance(unified_result, (list, dict)):
                results = unified_result if isinstance(unified_result, list) else [unified_result]
                return {
                    "success": True,
                    "results": results,
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                ...
                if self._metrics.query_count > 0 else 0.0
            ),
            "cache_hit_rate": (
...
def create_hybrid_engine_replacement(config: Optional['DatabaseConfig'] = None) -> HybridToUnifiedBridge:
    """
    ...
    if config:
        logger.info(f"Bridge initialized with config for engine type: {getattr(config, 'engine_type', 'unknown')}")
