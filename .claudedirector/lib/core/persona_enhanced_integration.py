...
class PersonaFrameworkOrchestrator:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        logger.info(
            "Persona Framework Orchestrator initialized",
    ...
    def _initialize_persona_manager(self):
        ...
        if PersonaManager is None:
            logger.warning(
                "PersonaManager not available, using enhanced framework only"
            )
            return None
    ...
    def analyze_with_enhanced_personas(
        self,
        user_input: str,
    ...
    def _determine_optimal_personas(
        self,
        user_input: str,
        ...
        for persona, preferred_frameworks in self.persona_framework_affinity.items():
            framework_overlap = set(frameworks_applied).intersection(
                set(preferred_frameworks)
            )
        ...
        for persona, keywords in content_scoring.items():
            ...
            persona_scores[persona] = (
                persona_scores.get(persona, 0) + content_score * 0.5
            )
    ...
    def _generate_persona_response(
        ...
        try:
            ...
            persona_context = {
                "frameworks_applied": enhanced_response.frameworks_applied,
                "confidence_score": enhanced_response.multi_framework_analysis.confidence_score,
                "strategic_themes": enhanced_response.multi_framework_analysis.integrated_insights.get(
                    "strategic_themes", []
                ),
                "recommendations": enhanced_response.context_aware_recommendations[:3],
                "implementation_focus": enhanced_response.multi_framework_analysis.implementation_roadmap.get(
                    "immediate_actions", []
                ),
            }
            ...
            return (
                ...
                if hasattr(persona_response, "response")
                else str(persona_response)
            )

        except Exception as e:
            logger.warning(
                "Persona response generation failed", persona=persona_name, error=str(e)
            )
            return self._generate_fallback_response(persona_name, enhanced_response)

    def _generate_fallback_response(
        self, persona_name: str, enhanced_response: EnhancedSystematicResponse
    ) -> str:
    ...
    def _generate_collaborative_suggestions(
        self,
        personas: List[str],
        enhanced_response: EnhancedSystematicResponse,
        user_input: str,
        ...
        if len(personas) > 1:
            ...
            collaboration_patterns = {
                (
                    "alvaro",
                    "rachel",
                ): "Consider how business strategy aligns with team dynamics and user experience",
                (
                    "alvaro",
                    "martin",
                ): "Evaluate the technical platform investment from a business ROI perspective",
                (
                    "rachel",
                    "martin",
                ): "Balance technical architecture decisions with team structure and user needs",
                (
                    "diego",
                    "alvaro",
                ): "Align engineering leadership initiatives with business strategic priorities",
                (
                    "camille",
                    "martin",
                ): "Integrate executive strategic vision with platform technical roadmap",
                (
                    "marcus",
                    "rachel",
                ): "Design change management approach that considers team culture and adoption patterns",
            }
        ...
        if len(frameworks) > 1:
            suggestions.append(
                f"The {frameworks[0]} and {frameworks[1]} frameworks provide complementary perspectives on this challenge"
            )

        # Add context-based suggestions
        if enhanced_response.conversation_continuity_notes:
            suggestions.append(
                "Build on insights from our previous discussions to maintain strategic momentum"
            )

        return suggestions

    def _extract_framework_integration(
        self, enhanced_response: EnhancedSystematicResponse
    ) -> Dict[str, Any]:
            ...
            "supporting_frameworks": [
                {
                    ...
                    "unique_contribution": framework.structured_insights.get(
                        "unique_insights", []
                    ),
                }
    ...
    def simple_persona_analysis(
        self, user_input: str, persona_name: str = "camille"
    ) -> str:
        """Simple persona analysis for backwards compatibility"""
        enhanced_response = self.analyze_with_enhanced_personas(
            user_input, requested_persona=persona_name
        )
...
class ConversationIntelligenceEngine:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        logger.info(
            "Conversation Intelligence Engine initialized",
    ...
    def process_conversation(
        ...
        response_package = {
            ...
            "frameworks_used": enhanced_response.framework_integration[
                "primary_framework"
            ]["name"],
        ...
        if self.collaborative_mode and enhanced_response.collaborative_suggestions:
            response_package["collaboration_opportunities"] = (
                enhanced_response.collaborative_suggestions
            )

        if self.context_retention and enhanced_response.conversation_continuity:
            response_package["context_continuity"] = (
                enhanced_response.conversation_continuity
            )

        return response_package

    def _format_for_chat_interface(
        self, enhanced_response: EnhancedPersonaResponse, user_input: str
    ) -> Dict[str, Any]:
        ...
        supporting_frameworks = enhanced_response.framework_integration.get(
            "supporting_frameworks", []
        )
        ...
        insights = {
            "confidence_level": (
                "high"
                if enhanced_response.confidence_score > 0.8
                else (
                    "medium" if enhanced_response.confidence_score > 0.6 else "moderate"
                )
            ),
            "frameworks_applied": len(
                enhanced_response.framework_integration.get("supporting_frameworks", [])
            )
            + 1,
            "strategic_depth": (
                "comprehensive"
                if len(
                    enhanced_response.framework_integration.get(
        ...
        return {"main_response": main_response, "conversation_insights": insights}
    ...
    def get_conversation_stats(self, session_id: str) -> Dict[str, Any]:
        ...
        context = self.orchestrator.enhanced_engine.memory_engine.get_or_create_context(
            session_id
        )
        insights = self.orchestrator.enhanced_engine.memory_engine.get_context_insights(
            session_id
        )
