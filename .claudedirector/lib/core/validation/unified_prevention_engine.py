...
class BloatModule:
    ...
    def validate(self, file_path: Path, content: str) -> ValidationResult:
        ...
        try:
            ...
            result.add_violation(e.lineno or 0, f"Syntax error: {e.msg}", "error", "syntax")
    ...
    def _check_hardcoded_values(self, content: str, result: ValidationResult):
        ...
        for i, line in enumerate(lines, 1):
            for pattern in self.hardcoded_patterns:
                if pattern.search(line) and 'CONFIG' not in line and 'CONSTANT' not in line:
                    result.add_warning(
                        i,
...
class P0Module:
    ...
    def validate(self, file_path: Path, content: str) -> ValidationResult:
        ...
        if any(keyword in str(file_path).lower() for keyword in ["strategic", "intelligence", "framework"]):
    ...
    def _validate_context7_usage(self, content: str, result: ValidationResult):
        ...
        context7_patterns = ["Context7", "pattern_access", "framework_pattern", "best_practice"]
    ...
    def _check_stakeholder_exposure(self, content: str, result: ValidationResult):
        ...
        for i, line in enumerate(lines, 1):
            ...
            for pattern in self.stakeholder_patterns:
                ...
                if matches and not any(word in line.lower() for word in ['example', 'test', 'demo']):
                    result.add_warning(
                        i,
...
class UnifiedPreventionEngine:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_module):
                ...
                try:
                    ...
                    logger.error(f"Module {module.get_name()} failed on {file_path}: {e}")
                    # Create error result
                    error_result = ValidationResult(module.get_name(), str(file_path))
                    error_result.add_violation(0, f"Module error: {str(e)}", "error", "module_error")
                    results[module.get_name()] = error_result
        
        return results
    
    def validate_directory(self, directory: Path, recursive: bool = True) -> Dict[str, Dict[str, ValidationResult]]:
    ...
    def generate_report(self, results: Dict[str, Dict[str, ValidationResult]]) -> str:
        ...
        for file_path, file_results in results.items():
            file_violations = sum(len(result.violations) for result in file_results.values())
            file_warnings = sum(len(result.warnings) for result in file_results.values())
            file_time = sum(result.execution_time_ms for result in file_results.values())
            ...
            if file_violations > 0 or file_warnings > 0:
                ...
                for module_name, result in file_results.items():
                    if result.violations or result.warnings:
                        report_lines.append(f"  🔍 {module_name} ({result.execution_time_ms:.1f}ms)")
                        
                        for violation in result.violations:
                            report_lines.append(f"    ❌ Line {violation['line']}: {violation['message']}")
                        
                        for warning in result.warnings:
                            report_lines.append(f"    ⚠️  Line {warning['line']}: {warning['message']}")
        ...
        report_lines.append(f"Average Time per File: {total_time/max(total_files, 1):.1f}ms")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Unified Prevention Engine - Consolidated Validation")
    parser.add_argument("path", help="File or directory to validate")
    parser.add_argument("--recursive", "-r", action="store_true", help="Recursive directory validation")
    parser.add_argument("--output", "-o", help="Output report to file")
    parser.add_argument("--modules", nargs="*", choices=["bloat", "p0", "security", "quality"], 
