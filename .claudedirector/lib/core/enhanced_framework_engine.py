...
from .embedded_framework_engine import EmbeddedFrameworkEngine, FrameworkAnalysis, SystematicResponse
...
class ConversationMemoryEngine:
    ...
    def __init__(self):
        ...
        self.topic_framework_mapping: Dict[str, Dict[str, int]] = defaultdict(lambda: defaultdict(int))
    ...
    def update_context(self, session_id: str, user_input: str, topics: List[str],
                      frameworks_used: List[str]) -> ConversationContext:
    ...
    def _analyze_complexity_progression(self, context: ConversationContext) -> Dict[str, Any]:
        ...
        for entry in context.conversation_history[-3:]:
            complexity_score = len(entry["input"]) / 100 + len(entry["frameworks"]) * 0.5
            recent_complexity.append(complexity_score)

        if len(recent_complexity) > 1:
            trend = "increasing" if recent_complexity[-1] > recent_complexity[0] else "stable"
        else:
            trend = "stable"

        return {
            "trend": trend,
            "current_complexity": "high" if recent_complexity[-1] > 2 else "medium" if recent_complexity[-1] > 1 else "low",
...
class MultiFrameworkIntegrationEngine:
    ...
    def _initialize_integration_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "strategic_analysis": {
                ...
                "output_structure": ["diagnosis", "policy", "actions", "platform_considerations"]
            },
            "organizational_design": {
                "frameworks": ["team_topologies", "accelerate_team_performance", "organizational_transformation"],
                "integration_focus": "team_structure_optimization",
                "output_structure": ["current_state", "team_patterns", "performance_metrics", "transformation_plan"]
            },
            "decision_making": {
                "frameworks": ["decisive_wrap_framework", "rumelt_strategy_kernel"],
                "integration_focus": "systematic_decision_process",
                "output_structure": ["options", "evidence", "distance", "preparation", "strategic_alignment"]
            },
            "scaling_leadership": {
                "frameworks": ["scaling_up_excellence", "crucial_conversations", "organizational_transformation"],
                "integration_focus": "leadership_scaling_strategy",
                "output_structure": ["excellence_spread", "communication_strategy", "change_management"]
            }
        }

    def identify_optimal_frameworks(self, user_input: str, context: ConversationContext) -> List[str]:
        ...
        supporting_frameworks = self._find_supporting_frameworks(primary_framework, user_input, context)

        # Limit to 3 frameworks maximum for manageable complexity
        all_frameworks = [primary_framework] + supporting_frameworks[:2]

        logger.info("Multi-framework selection",
    ...
    def _select_primary_framework(self, user_input: str, context: ConversationContext) -> str:
        ...
        if context.framework_usage_history:
            ...
            for framework, count in framework_frequency.most_common(2):
                ...
                if base_selection in synergistic:
                    logger.info("Framework selection enhanced by usage history",
                               base=base_selection, historic_pattern=framework)
                    break
    ...
    def _get_base_framework_recommendation(self, user_input: str) -> str:
        ...
        if any(word in input_lower for word in ["strategy", "strategic", "direction", "vision"]):
            return "rumelt_strategy_kernel"
        elif any(word in input_lower for word in ["decision", "choose", "options", "decide"]):
            return "decisive_wrap_framework"
        elif any(word in input_lower for word in ["platform", "architecture", "technical"]):
        ...
        elif any(word in input_lower for word in ["performance", "metrics", "velocity"]):
        ...
        elif any(word in input_lower for word in ["communication", "conversation", "conflict"]):
            return "crucial_conversations"
        else:
            return "rumelt_strategy_kernel"  # Default to strategy kernel

    def _find_supporting_frameworks(self, primary_framework: str, user_input: str,
    ...
    def _calculate_framework_relevance(self, framework: str, user_input: str,
        ...
        framework_keywords = {
            ...
            "strategic_platform_assessment": ["platform", "technical", "architecture", "infrastructure"],
            "organizational_transformation": ["change", "transform", "culture", "organization"],
            "team_topologies": ["team", "structure", "topology", "boundary"],
            "accelerate_team_performance": ["performance", "velocity", "metrics", "throughput"],
            "scaling_up_excellence": ["scaling", "growth", "excellence", "spread"],
            "crucial_conversations": ["communication", "conversation", "dialogue", "conflict"]
        }
        ...
        theme_alignment = self._check_theme_alignment(framework, context.strategic_themes)
    ...
    def _check_theme_alignment(self, framework: str, themes: Set[str]) -> float:
        ...
        framework_themes = {
            ...
            "strategic_platform_assessment": {"platform", "technology", "architecture", "technical"},
            "team_topologies": {"teams", "structure", "organization", "boundaries"},
            "accelerate_team_performance": {"performance", "metrics", "velocity", "delivery"},
            "scaling_up_excellence": {"scaling", "growth", "excellence", "standards"},
            "crucial_conversations": {"communication", "relationships", "conflict", "dialogue"}
        }
    ...
    def integrate_framework_analyses(self, analyses: List[FrameworkAnalysis],
                                   user_input: str, context: ConversationContext) -> MultiFrameworkAnalysis:
        ...
        comprehensive_recommendations = self._generate_comprehensive_recommendations(analyses, context)

        # Create implementation roadmap
        implementation_roadmap = self._create_implementation_roadmap(analyses, context)

        # Analyze stakeholder considerations
        stakeholder_considerations = self._analyze_stakeholder_considerations(analyses, context)
    ...
    def _combine_insights(self, analyses: List[FrameworkAnalysis]) -> Dict[str, Any]:
        ...
        for analysis in analyses:
            if "strategic_themes" in analysis.structured_insights:
                combined["strategic_themes"].extend(analysis.structured_insights["strategic_themes"])
        ...
        combined["common_patterns"] = [pattern for pattern, count in pattern_frequency.most_common(5) if count > 1]

        # Organize complementary perspectives
        for analysis in analyses:
            combined["complementary_perspectives"][analysis.framework_name] = {
                "unique_insights": analysis.structured_insights.get("unique_insights", []),
    ...
    def _identify_cross_patterns(self, analyses: List[FrameworkAnalysis]) -> List[str]:
        ...
        pattern_indicators = {
            "communication_focus": ["communication", "stakeholder", "alignment", "clarity"],
            "incremental_approach": ["phase", "gradual", "step", "incremental", "iterative"],
            "measurement_emphasis": ["metrics", "measure", "track", "monitor", "assess"],
            "risk_mitigation": ["risk", "mitigation", "contingency", "backup", "fallback"],
            "stakeholder_engagement": ["stakeholder", "engagement", "buy-in", "support", "involvement"]
        }

        for pattern_name, keywords in pattern_indicators.items():
            keyword_count = sum(1 for keyword in keywords if keyword in recommendation_text)
            if keyword_count >= 2:  # At least 2 keywords present
                patterns.append(f"Cross-framework emphasis on {pattern_name.replace('_', ' ')}")

        return patterns

    def _generate_comprehensive_recommendations(self, analyses: List[FrameworkAnalysis],
                                              context: ConversationContext) -> List[str]:
        ...
        context_recommendations = self._generate_context_specific_recommendations(analyses, context)
        recommendations.extend(context_recommendations)

        return recommendations[:8]  # Limit to top 8 recommendations

    def _group_recommendation_themes(self, recommendations: List[str]) -> Dict[str, List[str]]:
        """Group recommendations by similar themes"""
        themes = defaultdict(list)

        # Simple theme detection based on keywords
        theme_keywords = {
            "communication": ["communication", "stakeholder", "alignment", "clarity", "transparency"],
            ...
            "team_development": ["team", "skill", "capability", "training", "development"],
            "process_improvement": ["process", "workflow", "efficiency", "optimization"]
        }
    ...
    def _generate_context_specific_recommendations(self, analyses: List[FrameworkAnalysis],
                                                 context: ConversationContext) -> List[str]:
        ...
        if len(context.conversation_history) > 3:
            recommendations.append("Build on previous discussions to maintain strategic continuity")

        # Based on recurring themes
        if len(context.strategic_themes) > 2:
            theme_list = ", ".join(list(context.strategic_themes)[:3])
            recommendations.append(f"Consider integrated approach addressing recurring themes: {theme_list}")

        # Based on complexity progression
        insights = context.conversation_history[-1] if context.conversation_history else {}
        if insights and len(insights.get("frameworks", [])) > 1:
            recommendations.append("Leverage multi-framework insights for comprehensive strategic planning")

        return recommendations

    def _create_implementation_roadmap(self, analyses: List[FrameworkAnalysis],
                                     context: ConversationContext) -> Dict[str, List[str]]:
        ...
        for step in all_steps:
            ...
            if any(word in step_lower for word in ["immediate", "today", "now", "asap", "urgent"]):
                roadmap["immediate_actions"].append(step)
            elif any(word in step_lower for word in ["month", "quarter", "short", "soon", "next"]):
                roadmap["short_term_initiatives"].append(step)
            elif any(word in step_lower for word in ["year", "long", "strategic", "future", "vision"]):
    ...
    def _analyze_stakeholder_considerations(self, analyses: List[FrameworkAnalysis],
                                          context: ConversationContext) -> Dict[str, List[str]]:
        ...
        for analysis in analyses:
            ...
            for consideration in considerations:
                ...
                if any(word in consideration_lower for word in ["executive", "leadership", "ceo", "cto", "vp"]):
                    stakeholder_analysis["executive_leadership"].append(consideration)
                elif any(word in consideration_lower for word in ["engineering", "developer", "team", "technical"]):
                    stakeholder_analysis["engineering_teams"].append(consideration)
                elif any(word in consideration_lower for word in ["product", "pm", "user", "customer"]):
                    stakeholder_analysis["product_organization"].append(consideration)
                elif any(word in consideration_lower for word in ["external", "vendor", "partner", "client"]):
        ...
        if context.stakeholder_mentions:
            for stakeholder in context.stakeholder_mentions:
                stakeholder_analysis["executive_leadership"].append(f"Consider {stakeholder}'s perspective and requirements")

        return stakeholder_analysis

    def _calculate_confidence_score(self, analyses: List[FrameworkAnalysis], user_input: str) -> float:
        ...
        input_clarity = min(len(user_input.split()) / 20, 1.0)  # Longer inputs often clearer
    ...
    def _calculate_context_relevance(self, analyses: List[FrameworkAnalysis],
                                   context: ConversationContext) -> float:
        ...
        alignment_score = len(set(used_frameworks).intersection(historical_frameworks)) / max(len(used_frameworks), 1)
        ...
        theme_overlap = len(current_themes.intersection(context.strategic_themes)) / max(len(current_themes), 1)
        relevance += theme_overlap * 0.4

        # Conversation progression appropriateness
        if context.conversation_history:
            recent_complexity = len(context.conversation_history[-1].get("frameworks", []))
            current_complexity = len(analyses)

            if abs(current_complexity - recent_complexity) <= 1:  # Appropriate complexity progression
                relevance += 0.2
...
class EnhancedFrameworkEngine:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.context_window = self.config.get("context_window", 5)  # Number of previous interactions to consider

        logger.info("Enhanced Framework Engine initialized",
                   enhanced_mode=self.enhanced_mode,
                   max_frameworks=self.max_frameworks)

    def analyze_systematically(self, user_input: str, session_id: str = "default",
                             persona_context: Optional[Dict] = None) -> EnhancedSystematicResponse:
        """
        ...
        if self.enhanced_mode:
            selected_frameworks = self.integration_engine.identify_optimal_frameworks(user_input, context)
        else:
            # Fallback to single framework for backwards compatibility
            primary_framework = self.integration_engine._select_primary_framework(user_input, context)
        ...
        for framework_name in selected_frameworks:
            try:
                ...
                single_analysis = self.base_engine.analyze_systematically(user_input, framework_name)
                framework_analyses.append(single_analysis.analysis)
            except Exception as e:
                logger.warning("Framework analysis failed", framework=framework_name, error=str(e))
                continue
        ...
        if len(framework_analyses) > 1 and self.enhanced_mode:
            ...
            multi_analysis = MultiFrameworkAnalysis(
                ...
                implementation_roadmap={"immediate_actions": framework_analyses[0].implementation_steps},
        ...
        self.memory_engine.update_context(session_id, user_input, topics, selected_frameworks)
    ...
    def _extract_topics(self, user_input: str) -> List[str]:
        ...
        topic_patterns = {
            ...
            "performance_optimization": ["performance", "efficiency", "optimization", "velocity"],
    ...
    def _generate_persona_integrated_response(self, multi_analysis: MultiFrameworkAnalysis,
                                            frameworks: List[str],
                                            persona_context: Optional[Dict]) -> str:
        """Generate response that integrates analysis with persona characteristics"""
        # Base response on primary framework analysis
        base_response = multi_analysis.primary_framework.structured_insights.get("summary", "")

        # Enhance with multi-framework insights if available
        if multi_analysis.supporting_frameworks:
            framework_names = [f.framework_name for f in multi_analysis.supporting_frameworks]
        ...
        if multi_analysis.cross_framework_patterns:
            patterns_text = "\n\nInteresting patterns emerge across frameworks:\n" + \
                          "\n".join(f"• {pattern}" for pattern in multi_analysis.cross_framework_patterns[:3])
    ...
    def _generate_context_aware_recommendations(self, multi_analysis: MultiFrameworkAnalysis,
        ...
        if context_insights["recurring_themes"]:
            ...
            recommendations.append(f"Continue building on the recurring theme of {theme_name} from our discussions")

        # Based on conversation depth
        if context_insights["conversation_depth"] > 3:
            recommendations.append("Leverage insights from our ongoing strategic discussions for implementation")

        # Based on complexity progression
        complexity_info = context_insights["complexity_progression"]
        if complexity_info["trend"] == "increasing":
            recommendations.append("Consider breaking down complex initiatives into manageable phases")

        return recommendations

    def _generate_continuity_notes(self, context: ConversationContext,
                                 multi_analysis: MultiFrameworkAnalysis) -> List[str]:
        ...
        if context.framework_usage_history:
            ...
            if consistent_frameworks:
                ...
                notes.append(f"Maintaining consistency with {framework_names} approach from earlier conversations")

        # Reference strategic themes
        if len(context.strategic_themes) > 2:
            notes.append("Aligning with established strategic themes from our ongoing dialogue")

        return notes

    def _extract_learning_insights(self, user_input: str, multi_analysis: MultiFrameworkAnalysis,
                                 context: ConversationContext, frameworks: List[str]) -> Dict[str, Any]:
        ...
        for framework in frameworks:
            effectiveness_score = multi_analysis.confidence_score * multi_analysis.context_relevance
        ...
        input_characteristics = {
            ...
            "complexity_indicators": len([word for word in user_input.split() if len(word) > 8]),
        ...
        insights["conversation_patterns"] = {
            ...
            "framework_evolution": context.framework_usage_history[-3:] if context.framework_usage_history else [],
    ...
    def simple_analyze(self, user_input: str, framework_name: Optional[str] = None) -> SystematicResponse:
