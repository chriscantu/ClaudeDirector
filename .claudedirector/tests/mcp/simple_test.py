...
"""
    ...
    security_result = await server.execute_strategic_code(dangerous_code, "diego", {})

    print(
        f"✅ Security validation: {'BLOCKED' if not security_result.success else 'FAILED'}"
    )

    # Test integration layer
    integration = StrategicPythonMCPIntegration()
    print(
        f"✅ Integration initialized with {len(integration.server.capabilities)} capabilities"
    )
    ...
    disclosure = server.get_transparency_disclosure(
        "strategic_data_analysis", "diego", "basic calculation"
    )
    ...
    print(
        f"✅ Server info: {info['name']} with {len(info['capabilities'])} capabilities"
    )
