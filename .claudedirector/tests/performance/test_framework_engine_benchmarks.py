...
try:
    from claudedirector.frameworks.embedded_framework_engine import EmbeddedFrameworkEngine
...
class PerformanceBenchmarkFramework:
    ...
    def run_multiple_iterations(self, func: Callable, iterations: int = 10) -> Dict[str, Any]:
...
class TestFrameworkEnginePerformance:
    ...
    def test_framework_activation_performance(self):
        ...
        assert simple_stats['avg_time'] < 0.01, f"Simple context analysis too slow: {simple_stats['avg_time']}s"
        assert complex_stats['avg_time'] < 0.05, f"Complex context analysis too slow: {complex_stats['avg_time']}s"
        assert enterprise_stats['avg_time'] < 0.15, f"Enterprise context analysis too slow: {enterprise_stats['avg_time']}s"
        
        # Success rate assertions
        assert simple_stats['success_rate'] >= 0.99, f"Simple context success rate too low: {simple_stats['success_rate']}"
        assert complex_stats['success_rate'] >= 0.95, f"Complex context success rate too low: {complex_stats['success_rate']}"
        assert enterprise_stats['success_rate'] >= 0.90, f"Enterprise context success rate too low: {enterprise_stats['success_rate']}"
    ...
    def test_framework_accuracy_performance(self):
        ...
        strategic_contexts = [
            ("We need to decide between two technology platforms", ["Decisive WRAP", "Rumelt Strategy"]),
            ("Our teams are struggling with communication and collaboration", ["Team Topologies", "Crucial Conversations"]),
            ("We need to improve our development velocity and quality", ["Accelerate", "Team Topologies"]),
            ("Budget allocation for next year's platform investments", ["Capital Allocation", "Rumelt Strategy"]),
            ("Implementing a major organizational change", ["Scaling Up Excellence", "Crucial Conversations"]),
            ("Stakeholder alignment on strategic priorities", ["Crucial Conversations", "Rumelt Strategy"]),
            ("Platform architecture and team boundaries", ["Team Topologies", "Decisive WRAP"])
        ]
        ...
        def benchmark_framework_accuracy():
            ...
            for context, expected_frameworks in strategic_contexts:
                ...
                accuracy = any(framework in activated for framework in expected_frameworks)
        ...
        # Accuracy assertions
        assert accuracy_stats['avg_time'] < 0.1, f"Framework accuracy check too slow: {accuracy_stats['avg_time']}s"
        assert accuracy_stats['success_rate'] >= 0.95, f"Framework accuracy success rate too low: {accuracy_stats['success_rate']}"
    ...
    def test_framework_scalability_performance(self):
        ...
        assert scalability_stats['avg_time'] < 0.2, f"Concurrent analysis too slow: {scalability_stats['avg_time']}s"
        assert scalability_stats['success_rate'] >= 0.90, f"Concurrent analysis success rate too low: {scalability_stats['success_rate']}"
    ...
    def test_framework_recommendation_performance(self):
        ...
        assert recommendation_stats['avg_time'] < 0.08, f"Recommendation generation too slow: {recommendation_stats['avg_time']}s"
        assert recommendation_stats['success_rate'] >= 0.92, f"Recommendation success rate too low: {recommendation_stats['success_rate']}"
        
        # Store results
        self.benchmark.results['framework_recommendations'] = recommendation_stats
        
        print(f"\n💡 Framework Recommendation Performance:")
        print(f"Recommendation generation (avg): {recommendation_stats['avg_time']:.4f}s")
    ...
    def test_memory_usage_monitoring(self):
        ...
        try:
            ...
            assert memory_increase < 25, f"Memory usage increased too much: {memory_increase:.2f}MB"
