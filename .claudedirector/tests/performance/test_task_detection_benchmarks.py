...
class TestTaskDetectionPerformance:
    ...
    def test_task_detection_response_time(self, mock_detector, mock_manager):
        ...
        assert simple_stats['avg_time'] < 0.05, f"Simple task detection too slow: {simple_stats['avg_time']}s"
        assert complex_stats['avg_time'] < 0.15, f"Complex task detection too slow: {complex_stats['avg_time']}s"
        assert enterprise_stats['avg_time'] < 0.5, f"Enterprise task detection too slow: {enterprise_stats['avg_time']}s"
        
        # Success rate assertions
        assert simple_stats['success_rate'] >= 0.95, f"Simple task success rate too low: {simple_stats['success_rate']}"
        assert complex_stats['success_rate'] >= 0.90, f"Complex task success rate too low: {complex_stats['success_rate']}"
        assert enterprise_stats['success_rate'] >= 0.85, f"Enterprise task success rate too low: {enterprise_stats['success_rate']}"
    ...
    def test_task_priority_detection_performance(self, mock_detector, mock_manager):
        ...
        mock_detector_instance.detect_tasks.return_value = [
            {"task": "Critical security fix", "confidence": 0.95, "priority": "critical"},
        ...
        priority_stats = self.benchmark.run_multiple_iterations(
            benchmark_priority_detection,
            iterations=30
        )
        
        # Performance assertions for priority detection
        assert priority_stats['avg_time'] < 0.2, f"Priority detection too slow: {priority_stats['avg_time']}s"
        assert priority_stats['success_rate'] >= 0.90, f"Priority detection success rate too low: {priority_stats['success_rate']}"
    ...
    def test_task_storage_performance(self, mock_manager):
        ...
        assert retrieval_stats['avg_time'] < 0.01, f"Task retrieval too slow: {retrieval_stats['avg_time']}s"
        assert retrieval_stats['success_rate'] >= 0.98, f"Task retrieval success rate too low: {retrieval_stats['success_rate']}"
