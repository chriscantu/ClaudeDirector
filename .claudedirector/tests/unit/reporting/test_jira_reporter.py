...
class TestJiraReporterDurationLabels:
    ...
    def test_duration_label_quarterly(self):
        ...
        assert label in [
            "91-Day",
            "Quarterly",
        ], "91 days should map to quarterly format"
...
class TestJiraReporterInitialization:
    ...
    def test_duration_parameter_defaults_to_7(self):
        ...
        mock_config.config = {
            "jira": {
                ...
                "auth": {"email": "test@test.com", "api_token": "test"},
            },
            "jql_queries": {},
        }
    ...
    def test_duration_label_set_on_init(self):
        ...
        assert hasattr(
            reporter, "duration_label"
        ), "Reporter should have duration_label attribute"
...
class TestJiraReporterDateRangeCalculation:
    ...
    def test_calculate_date_range_weekly(self):
        ...
        assert (
            abs((start_date - expected_start).total_seconds()) < 1
    ...
    def test_calculate_date_range_90day(self):
        ...
        assert (
            abs((start_date - expected_start).total_seconds()) < 1
        ), "Start date should be 90 days before end date"
...
class TestBLOATPrevention:
    ...
    def test_jira_reporter_module_exists(self):
        ...
        assert (
            jira_reporter_path.exists()
        ), f"jira_reporter.py should exist at {jira_reporter_path}"
    ...
    def test_no_duplicate_duration_logic_in_weekly_reporter(self):
        ...
        if weekly_reporter_path.exists():
            ...
            assert (
                "from .jira_reporter import" in content
                or "from reporting.jira_reporter import" in content
    ...
    def test_single_source_of_truth_for_duration(self):
        ...
        assert hasattr(
            JiraReporter, "_get_duration_label"
...
class TestBackwardCompatibility:
    ...
    def test_weekly_reporter_still_importable(self):
        ...
        try:
            ...
            assert (
                weekly_reporter is not None
            ), "weekly_reporter module should still exist"
        except ImportError as e:
            pytest.fail(
                f"weekly_reporter should remain importable for backward compatibility: {e}"
            )
    ...
    def test_weekly_reporter_classes_available(self):
        ...
        try:
            ...
            sys.path.insert(0, str(Path(__file__).parent.parent.parent))
            from lib.reporting.weekly_reporter import (
                JiraClient,
                StrategicAnalyzer,
                ConfigManager,
            )
            ...
            assert (
                StrategicAnalyzer is not None
            ), "StrategicAnalyzer should be available"
            assert ConfigManager is not None, "ConfigManager should be available"
        except ImportError as e:
            pytest.fail(
                f"Core classes should remain importable from weekly_reporter: {e}"
            )
