            ...
            for category, scenarios in self.test_scenarios.items():
                for scenario in scenarios:
                    try:
                        ...
                        print(f"⚠️ Framework detection failed for scenario: {scenario[:50]}... - {e}")
            
            # Verify we detected multiple frameworks
            self.assertGreater(
                len(detected_frameworks), 3,
    ...
    def test_framework_selection_consistency(self):
        ...
        try:
                ...
                try:
                    ...
                    if result:
                        if hasattr(result, 'framework_name'):
                            ...
                            framework_names = [f.name if hasattr(f, 'name') else str(f) for f in result]
                        else:
            ...
            if len(results) >= 2:
                ...
                self.assertGreater(
                    len(results[0]) if results[0] else 0, 0,
        ...
        try:
            ...
            for scenario in test_scenarios:
                ...
                try:
                    ...
                    print(f"⚠️ Performance test failed for scenario: {scenario[:30]}... - {e}")
...
class TestFrameworkDataIntegrity(unittest.TestCase):
    ...
    def test_framework_knowledge_preservation(self):
        ...
        try:
            ...
            strategic_input = "We need to make a strategic decision about our technology platform"
            result = engine.analyze_systematically(
                strategic_input, 
                persona_context="martin", 
                domain_focus=["strategic", "technical"]
...
def run_framework_engine_regression_tests():
    ...
    suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestFrameworkDetectionRegression))
    suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestFrameworkConfigurationIntegration))
    suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestFrameworkDataIntegrity))
