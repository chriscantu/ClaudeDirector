...
class TestConversationalAnalyticsP0:
    ...
    async def test_p0_query_parsing_team_performance(self, data_manager):
        ...
        assert parsed_query is not None
        assert parsed_query.query_type.value in [
            "team_performance",
            "general_analytics",
        ]
    ...
    async def test_p0_data_fetching_performance(self, data_manager):
        ...
        parsed_query, data_response = await data_manager.process_conversational_query(
            query_text
        )
        ...
        assert (
            total_time < 5000
        ), f"Data fetch took {total_time:.2f}ms, exceeds 5s limit"
    ...
    async def test_p0_chat_embedded_visualization_generation(self, workflow):
        ...
        assert (
            "claudedirector-chat-visualization"
            in result.visualization_result.html_output
        )
        assert (
            result.visualization_result.generation_time < 1.0
        )  # <1s for visualization
    ...
    async def test_p0_context_preservation(self, workflow):
        ...
        result2 = await workflow.process_chat_query(
            query2, context=context_with_history
        )
    ...
    async def test_p0_error_handling_graceful_degradation(self, workflow):
        ...
        assert (
            len(result.visualization_result.html_output) > 0
    ...
    async def test_p0_performance_targets_met(self, workflow):
        ...
        assert (
            result.visualization_result.generation_time < 0.5
        ), "Visualization exceeds 500ms"
