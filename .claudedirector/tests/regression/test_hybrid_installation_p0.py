...
class TestHybridInstallationP0(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from core.hybrid_installation_manager import HybridInstallationManager, InstallationResult
    ...
    def test_p0_zero_setup_principle_maintained(self):
        ...
        with patch.object(Path, 'home', return_value=Path(self.temp_dir)):
            ...
            with patch('subprocess.run') as mock_run:
                ...
                result = manager.install_mcp_package("@modelcontextprotocol/server-sequential")

                # CRITICAL: Must succeed even without permanent installation
                self.assertTrue(result.success, "Zero setup principle violated - installation must succeed")
                self.assertEqual(result.method, "temporary", "Must fall back to temporary installation")
                self.assertTrue(result.command_available, "Command must be available after installation")

                print("✅ Zero setup principle maintained - temporary installation works")
    ...
    def test_p0_performance_optimization_detection(self):
        ...
        with patch.object(Path, 'home', return_value=Path(self.temp_dir)):
            ...
            with patch('subprocess.run') as mock_run:
                ...
                self.assertTrue(result.success, "Performance optimization detection failed")
                self.assertEqual(result.method, "permanent", "Must detect permanent installation")
                self.assertLess(result.duration, 1.0, "Permanent installation must be fast")

                print(f"✅ Performance optimization detected - {result.method} installation in {result.duration:.3f}s")
        ...
        with patch.object(Path, 'home', return_value=Path(self.temp_dir)):
            ...
            with patch.object(manager, '_check_permanent_installation') as mock_check:
                mock_check.return_value = self.InstallationResult(
                    success=True, method="permanent", duration=0.46, command_available=True
                )

                message = manager.get_installation_strategy("test-package")
                self.assertIn("optimized", message.lower(), "Must indicate optimization")
                self.assertIn("faster", message.lower(), "Must indicate performance benefit")

            # Test temporary installation message
            with patch.object(manager, '_check_permanent_installation') as mock_check:
                mock_check.return_value = self.InstallationResult(
                    success=False, method="permanent", duration=0.5, command_available=False
                )

                message = manager.get_installation_strategy("test-package")
                self.assertIn("installing", message.lower(), "Must indicate installation")
                self.assertIn("enhancement", message.lower(), "Must indicate MCP enhancement")
    ...
    def test_p0_usage_tracking_accuracy(self):
        ...
        with patch.object(Path, 'home', return_value=Path(self.temp_dir)):
            ...
            self.assertEqual(manager.usage_stats.total_uses, 0, "Initial usage count must be zero")
            self.assertEqual(manager.usage_stats.temporary_install_uses, 0, "Initial temporary uses must be zero")

            # Simulate successful installation
            manager._update_usage_stats(True)

            # Verify tracking
            self.assertEqual(manager.usage_stats.total_uses, 1, "Total uses must increment")
            self.assertEqual(manager.usage_stats.temporary_install_uses, 1, "Temporary uses must increment")

            # Test persistence
            stats_data = json.loads(manager.usage_file.read_text())
            self.assertEqual(stats_data["total_uses"], 1, "Usage stats must persist correctly")
    ...
    def test_p0_performance_hint_logic(self):
        ...
        with patch.object(Path, 'home', return_value=Path(self.temp_dir)):
            ...
            self.assertFalse(manager._should_show_performance_hint(), "Hint should not show for low usage")

            # Test hint shown for high usage
            manager.usage_stats.temporary_install_uses = 5
            self.assertTrue(manager._should_show_performance_hint(), "Hint should show for high usage")

            # Test hint throttling (not shown again within 24 hours)
            manager.usage_stats.last_hint_shown = time.time() - 3600  # 1 hour ago
            self.assertFalse(manager._should_show_performance_hint(), "Hint should be throttled")

            # Test hint shown after 24 hours
            manager.usage_stats.last_hint_shown = time.time() - 86401  # 24+ hours ago
            self.assertTrue(manager._should_show_performance_hint(), "Hint should show after 24 hours")
    ...
    def test_p0_fallback_chain_ordering(self):
        ...
        with patch.object(Path, 'home', return_value=Path(self.temp_dir)):
            ...
            with patch('subprocess.run') as mock_run:
                ...
                self.assertEqual(call_count, 3, "Must attempt all steps in fallback chain")
                self.assertTrue(result.success, "Fallback chain must succeed")
                self.assertEqual(result.method, "temporary", "Must use correct fallback method")
    ...
    def test_p0_error_handling_graceful_degradation(self):
        ...
        with patch.object(Path, 'home', return_value=Path(self.temp_dir)):
            ...
            with patch('subprocess.run') as mock_run:
                ...
                self.assertIn("failed", result.error_message.lower(), "Error message must be descriptive")
    ...
    def test_p0_performance_thresholds(self):
        ...
        with patch.object(Path, 'home', return_value=Path(self.temp_dir)):
            ...
            self.assertLessEqual(manager.permanent_install_time, 0.5, "Permanent install must be ≤0.5s")
            self.assertLessEqual(manager.temporary_install_time, 1.2, "Temporary install must be ≤1.2s")

            # Verify performance improvement calculation
            improvement = (manager.temporary_install_time - manager.permanent_install_time) / manager.temporary_install_time
            self.assertGreaterEqual(improvement, 0.5, "Must provide ≥50% performance improvement")
    ...
    def test_p0_backwards_compatibility(self):
        ...
        with patch.object(Path, 'home', return_value=Path(self.temp_dir)):
            ...
            from core.hybrid_installation_manager import get_hybrid_manager, install_mcp_package
            ...
            with patch('subprocess.run') as mock_run:
                ...
                self.assertIsInstance(result, self.InstallationResult, "Must return InstallationResult")
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                with patch.object(Path, 'home', return_value=Path(temp_dir)):
                    ...
                    config = get_config()
                    self.assertIsNotNone(manager.config, "Must integrate with configuration system")
                    self.assertEqual(manager.config, config, "Must use same configuration instance")
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                with patch.object(Path, 'home', return_value=Path(temp_dir)):
                    ...
                    try:
                        ...
                        result = manager._check_permanent_installation("nonexistent-package-test")
                        duration = time.time() - start_time

                        # Should complete quickly regardless of result
                        self.assertLess(duration, 5.0, "Performance check must complete within 5s")
...
def run_hybrid_installation_p0_tests():
    ...
    suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestHybridInstallationIntegration))
