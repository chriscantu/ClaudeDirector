...
class TestPersonaRegression:
    ...
    def test_diego_standard_responses(self, sample_inputs):
        ...
        for input_text in diego_inputs:
            ...
            assert any(indicator in response.lower() for indicator in [
                "coordinate", "align", "facilitate", "team", "collaboration"
            ]) or "mock" in response  # Allow for mock responses in testing
    ...
    def test_persona_response_times(self, sample_inputs, baseline_response_times):
        ...
        for persona, inputs in sample_inputs.items():
            for input_text in inputs:
                ...
                assert response_time < max_standard_time, \
    ...
    def test_mcp_integration_graceful_degradation(self):
        ...
        with patch('claudedirector.integrations.mcp_use_client.MCPUseClient') as mock_client:
    ...
    def test_performance_regression_detection(self, sample_inputs):
        ...
        for persona, inputs in sample_inputs.items():
            ...
            assert avg_response_time < 1.0, f"{persona} avg response time too high: {avg_response_time:.2f}s"
            assert max_response_time < 2.0, f"{persona} max response time too high: {max_response_time:.2f}s"
