...
class TestConfigurationIntegrity(unittest.TestCase):
    ...
    def test_config_module_exists_and_importable(self):
        ...
        try:
            ...
            self.assertTrue(
                hasattr(config_module, 'ClaudeDirectorConfig') or 
                hasattr(config_module, 'get_config') or
                len([attr for attr in dir(config_module) if not attr.startswith('_')]) > 0,
    ...
    def test_all_hardcoded_values_catalogued(self):
        ...
        self.assertGreater(
            len(hardcoded_values["strings"]), 50,
            "Should find substantial number of hard-coded strings"
        )
        
        self.assertGreater(
            len(hardcoded_values["numbers"]), 20,
    ...
    def test_configuration_access_patterns(self):
        ...
        try:
            ...
            self.assertTrue(
                hasattr(config, 'get') or 
                hasattr(config, '__getitem__') or
                hasattr(config, 'get_threshold'),
    ...
    def test_threshold_value_consistency(self):
        ...
        duplicate_thresholds = {
            k: v for k, v in common_thresholds.items() 
            if len(v) > 1
        }
        
        self.assertGreater(
            len(duplicate_thresholds), 0,
    ...
    def test_string_pattern_consistency(self):
        ...
        for pattern in ["strategic", "organizational", "technical", "stakeholder"]:
            files_using_pattern = [
                f for f, patterns in string_patterns.items() 
        ...
        for pattern, usage_count in pattern_usage.items():
            self.assertGreater(
                usage_count, 3,
                f"Pattern '{pattern}' should be used in multiple files"
            )
    ...
    def test_configuration_validation_requirements(self):
        ...
        print(f"   Threshold ranges: {len(validation_requirements['threshold_ranges'])}")
        ...
        for py_file in self.core_path.glob("*.py"):
            ...
            try:
                ...
                for match in string_matches:
                    if any(pattern in match.lower() for pattern in 
                          ["strategic", "technical", "stakeholder", "decision", "risk"]):
    ...
    def _extract_string_patterns(self) -> Dict[str, List[str]]:
        ...
        for py_file in self.core_path.glob("*.py"):
            ...
            try:
                ...
                patterns = []
                for pattern in ["strategic", "organizational", "technical", "stakeholder", "decision", "risk"]:
...
class TestConfigurationBackwardsCompatibility(unittest.TestCase):
    ...
    def test_existing_api_preserved(self):
        ...
        print(f"   Import paths to preserve: {len(compatibility_requirements['import_paths'])}")
        print(f"   Method signatures to maintain: {len(compatibility_requirements['method_signatures'])}")
...
def run_configuration_integrity_tests():
    ...
    suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestConfigurationIntegrity))
    suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestConfigurationBackwardsCompatibility))
