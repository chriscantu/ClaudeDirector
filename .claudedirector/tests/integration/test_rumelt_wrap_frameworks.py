...
def test_rumelt_framework_direct():
    ...
    rumelt_keywords = ["diagnosis", "policy", "action", "challenge", "leverage", "fluff", "coherent"]
    ...
    print(f"   Rumelt Elements Found: {len(found_rumelt_elements)}/{len(rumelt_keywords)}")
    ...
    # Evaluate success
    if "Rumelt" in framework_analysis.framework_name and len(found_rumelt_elements) >= 3:
...
def test_wrap_framework_direct():
    ...
    wrap_keywords = ["widen", "reality", "distance", "prepare", "options", "assumptions", "evidence", "test"]
    found_wrap_elements = []
...
def test_persona_integration():
    ...
    diego_markers = ["😊", "excited", "systematic", "collaborate", "resonates", "thoughts"]
    response_lower = systematic_response.persona_integrated_response.lower()
    found_diego_markers = [marker for marker in diego_markers if marker.lower() in response_lower]
    
    print(f"   Diego Personality Markers: {len(found_diego_markers)}/{len(diego_markers)}")
    print(f"   Found: {', '.join(found_diego_markers)}")
    
    # Show response preview
    response_preview = systematic_response.persona_integrated_response[:300] + "..." if len(systematic_response.persona_integrated_response) > 300 else systematic_response.persona_integrated_response
    print(f"\n📋 **Response Preview:**")
