...
def create_demo_enhanced_manager() -> EnhancedPersonaManager:
    ...
    manager = EnhancedPersonaManager(
        template_discovery=template_discovery, enhancement_config=enhancement_config
    )
...
def demo_diego_systematic_analysis():
    ...
    test_input = (
        "Help me develop Q4 platform strategy with systematic stakeholder alignment"
    )
    ...
    print(f"\n✅ **Results:**")
    print(
        f"   Persona Selected: {result.get('persona_selection', {}).get('primary', 'Unknown')}"
    )
    print(
        f"   Enhancement Applied: {result.get('enhancement_result', {}).get('enhancement_applied', False)}"
    )
    print(
        f"   Framework Used: {result.get('enhancement_result', {}).get('framework_used', 'None')}"
    )
    ...
    found_indicators = [
        word
        for word in systematic_indicators
    ...
    found_characteristics = [
        char
        for char in diego_characteristics
...
def demo_persona_enhancement_engine():
    ...
    enhancement_result = enhancement_engine.enhance_response(
        persona_name="diego", user_input=test_input, base_response=base_response
    )
...
def demo_rumelt_wrap_frameworks():
    ...
    for scenario in test_scenarios:
        ...
        if enhancement_result.enhancement_applied:
            response_preview = (
                enhancement_result.enhanced_response[:200] + "..."
                if len(enhancement_result.enhanced_response) > 200
                else enhancement_result.enhanced_response
            )
            ...
            expected_keywords = framework_keywords.get(
                scenario["expected_framework"], []
            )
            found_keywords = [
                kw
                for kw in expected_keywords
                if kw.lower() in enhancement_result.enhanced_response.lower()
            ]

            print(
                f"   - Framework Keywords Found: {len(found_keywords)}/{len(expected_keywords)} ({', '.join(found_keywords)})"
            )
        else:
