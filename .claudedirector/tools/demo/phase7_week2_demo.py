...
async def demo_conversational_query(
    workflow: ConversationalAnalyticsWorkflow, query: str, persona: str = "diego"
):
...
async def main():
    ...
    for i, demo in enumerate(demo_queries, 1):
        ...
        result = await demo_conversational_query(
            workflow, demo["query"], demo["persona"]
        )
    ...
    avg_visualization = sum(
        r.visualization_result.generation_time * 1000 for r in results
    ) / len(results)

    print(
        f"✅ **Success Rate**: {success_rate:.1%} ({len(successful_queries)}/{total_queries})"
    )
    ...
    print(
        f"💬 **Chat-Only Interface**: {'✅ PASS' if all_chat_optimized else '❌ FAIL'}"
    )
    print(
        f"🎭 **Magic MCP Integration**: {'✅ PASS' if all_magic_mcp_ready else '❌ FAIL'}"
    )
    print(f"⚡ **<5s Response Time**: {'✅ PASS' if avg_latency < 5000 else '❌ FAIL'}")
    print(
        f"🎨 **<500ms Visualization**: {'✅ PASS' if avg_visualization < 500 else '❌ FAIL'}"
    )
    ...
    result1 = await demo_conversational_query(
        workflow, "Show me sprint metrics for platform team", "diego"
    )
    ...
    print(
        f"✅ **Context Preserved**: {result2.pipeline_metadata.get('context_provided', False)}"
    )
