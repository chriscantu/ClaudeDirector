#!/bin/bash
# Pre-push hook: Complete GitHub CI Workflow Validation with Architectural Validation
# Runs the entire CI pipeline locally before pushing to prevent GitHub CI failures

set -e  # Exit on any error

echo "🚀 PRE-PUSH CI VALIDATION WITH ARCHITECTURAL VALIDATION"
echo "============================================================"
...
if [ -f ".git/hooks/pre-push-sanity-check" ]; then
    .git/hooks/pre-push-sanity-check "$@"
fi

print_section "PHASE 0: Architectural Validation"

# NEW: Architectural Validation
run_step "Architectural Validation" "
echo '🏗️ ARCHITECTURAL VALIDATION'
echo 'Validating ClaudeDirector architectural principles...'
python .claudedirector/tools/git-hooks/architectural-validation-focused.py
...
# 7. P0 Feature Test Suite - UPDATED PATHS
...
# HIGH PRIORITY P0 TESTS (Should pass but won't block CI) - UPDATED PATHS
echo '🧪 RUNNING HIGH PRIORITY P0 TEST: First-Run Wizard P0'
timeout 60 python .claudedirector/tests/integration/test_first_run_wizard_comprehensive.py > /dev/null 2>&1 || (
...
timeout 60 python .claudedirector/tests/integration/test_cursor_integration_comprehensive.py > /dev/null 2>&1 || (
...
print_status $GREEN "  ✅ Architectural Validation: PASSED"
