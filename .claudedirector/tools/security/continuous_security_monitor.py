...
class ContinuousSecurityMonitor:
    ...
    def run_continuous_monitoring(self, duration_minutes: int = 60):
        ...
        """
        print(
            f"🛡️ Starting continuous security monitoring for {duration_minutes} minutes"
        )
        ...
        while datetime.now() < end_time:
            try:
                ...
                if scan_result["threats_detected"] > 0:
                    monitoring_session["threats_detected"] += scan_result[
                        "threats_detected"
                    ]
        ...
        print(
            f"🛡️ Monitoring session completed. Scans: {monitoring_session['scans_performed']}, Alerts: {monitoring_session['alerts_generated']}"
        )
    ...
    def _log_status(self, scan_result: Dict):
        ...
        status = {
            ...
            "status": (
                "SECURE" if scan_result["threats_detected"] == 0 else "THREATS_DETECTED"
            ),
        }
    ...
    def _update_metrics(self, scan_result: Dict):
        ...
        metrics = {
            ...
            "threat_detection_rate": (
                1.0 if scan_result["threats_detected"] > 0 else 0.0
            ),
    ...
    def _generate_session_summary(self, session: Dict):
        ...
        summary_file = (
            self.monitor_dir / f"session_summary_{session['session_id']}.json"
        )
        ...
        session["average_scan_interval"] = session["duration_minutes"] / max(
            1, session["scans_performed"]
        )
        session["threat_detection_rate"] = session["threats_detected"] / max(
            1, session["scans_performed"]
        )
    ...
    def _generate_alert_id(self) -> str:
        ...
        return hashlib.md5(f"alert_{datetime.now().isoformat()}".encode()).hexdigest()[
            :8
        ]
