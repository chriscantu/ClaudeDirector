...
class StakeholderAIManager:
    ...
    def process_workspace_automatically(self):
        ...
        for meeting_dir in workspace_dir.iterdir():
            if meeting_dir.is_dir():
                ...
                content_files = list(meeting_dir.glob("*.md")) + list(
                    meeting_dir.glob("*.txt")
                )
                ...
                for content_file in content_files:
                    try:
                        ...
                        result = self.detector.process_content_for_stakeholders(
                            content, context
                        )

                        if result["candidates_detected"] > 0:
                            print(
                                f"   📊 Detected {result['candidates_detected']} stakeholder candidates"
                            )
                            print(f"   ✅ Auto-created: {result['auto_created']}")
                            print(
                                f"   ❓ Profiling needed: {result['profiling_needed']}"
                            )
                            print(
                                f"   🔄 Updates suggested: {result['updates_suggested']}"
                            )
        ...
        if question.get("options"):
            ...
            if question.get("multiple_choice"):
                ...
                try:
                    ...
                    return (
                        [question.get("pre_filled")]
                        if question.get("pre_filled")
                        else []
                    )
            else:
                while True:
    ...
    def _build_profile_from_responses(self, task: Dict, responses: Dict) -> Dict:
        ...
        profile = {
            ...
            "role_title": None,
            "strategic_importance": task["auto_detected_info"].get(
                "importance", "medium"
            ),
        ...
        if "role_confirmation" in responses:
            if responses["role_confirmation"] == "yes":
                profile["role_title"] = (
                    task["auto_detected_info"].get("role", "").title()
                )
    ...
    def show_ai_summary(self):
        ...
        if profiling_tasks:
            print(
                "   1. Run 'python stakeholder_ai_manager.py profile' to complete profiling"
            )
        if update_suggestions:
            print(
                "   2. Run 'python stakeholder_ai_manager.py updates' to review updates"
            )
