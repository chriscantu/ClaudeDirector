...
Engineering Leadership: Cross-session strategic context persistence
...
class SuperClaudeMemoryManager:
    ...
    def store_executive_session(self, session: ExecutiveSession) -> int:
        ...
        logger.info(
            f"Stored executive session: {session.stakeholder_key} ({session_id})"
        )
        return session_id

    def get_stakeholder_context(
        self, stakeholder_key: str, days: int = 90
    ) -> Dict[str, Any]:
            ...
            sessions.append(
                {
                    ...
                    "agenda_topics": (
                        json.loads(row["agenda_topics"]) if row["agenda_topics"] else []
                    ),
                    "decisions_made": (
                    ...
                    "action_items": (
                        json.loads(row["action_items"]) if row["action_items"] else []
                    ),
        ...
        cursor.execute(
            "SELECT * FROM stakeholder_profiles WHERE stakeholder_key = ?",
            (stakeholder_key,),
        )
        ...
        if profile_row:
            profile = {
                ...
                "preferred_personas": (
                    json.loads(profile_row["preferred_personas"])
    ...
    def get_initiative_context(self, initiative_key: str) -> Dict[str, Any]:
        ...
        cursor.execute(
            "SELECT * FROM strategic_initiatives WHERE initiative_key = ?",
            (initiative_key,),
        )
        row = cursor.fetchone()
        ...
        initiative = dict(row)
        if initiative["resource_allocation"]:
            initiative["resource_allocation"] = json.loads(
    ...
    def store_platform_metric(self, intelligence: PlatformIntelligence) -> None:
        ...
        self.conn.commit()
        logger.info(
            f"Stored platform metric: {intelligence.category}/{intelligence.metric_name}"
        )

    def get_platform_trends(
        self, category: str = None, days: int = 90
    ) -> List[Dict[str, Any]]:
...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["stats", "recall", "cleanup", "test"],
        help="Memory operation to perform",
    )
