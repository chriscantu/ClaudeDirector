...
from .db_base import (
    DatabaseEngineBase, WorkloadType, QueryContext, DatabaseConfig, 
    DatabaseResult, OptimizationLevel
)
...
class HybridDatabaseEngine(DatabaseEngineBase):
    ...
    def __init__(self, config: DatabaseConfig):
        ...
        self._workload_metrics = WorkloadMetrics(
            read_ratio=0.8, write_ratio=0.2, avg_result_size=10,
            join_frequency=0.3, aggregation_frequency=0.1, 
            json_query_frequency=0.4, concurrent_connections=1, 
            query_frequency=1.0
        )
        ...
        self.logger.info("Hybrid database engine initialized",
                        sqlite_enabled=True,
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if self._should_enable_duckdb():
                try:
                    ...
                    if not self._duckdb_engine.connect():
                        self.logger.warning("DuckDB connection failed, continuing with SQLite only")
            ...
            if self._should_enable_faiss():
                try:
                    ...
                    if not self._faiss_engine.connect():
                        self.logger.warning("Faiss connection failed, semantic search unavailable")
            ...
            self.logger.info("Hybrid database connected successfully",
    ...
    def execute_query(self, query: str, params: Optional[Dict] = None,
        ...
        try:
            ...
            workload_type = context.workload_type if context else self._infer_workload_type(query)
            ...
            execution_time = (time.time() - start_time) * 1000
            self.logger.error("Hybrid query execution failed",
                            query=query[:100],
                            execution_time_ms=execution_time,
                            error=str(e))
            
            return DatabaseResult(
                success=False, data=[], execution_time_ms=int(execution_time),
    ...
    def optimize_performance(self, level: OptimizationLevel) -> bool:
        ...
        try:
            ...
            self.logger.info("Hybrid database optimization completed",
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        engine_statuses = [engine['status'] for engine in health_status['engines'].values()]
    ...
    def _classify_query_complexity(self, query: str) -> QueryComplexity:
        ...
        if any(pattern in query_upper for pattern in [
            'GROUP BY', 'WINDOW FUNCTION', 'WITH RECURSIVE', 'PARTITION BY',
            'PERCENTILE', 'LAG(', 'LEAD(', 'ROW_NUMBER()'
        ]):
        ...
        if join_count > 0 or any(agg in query_upper for agg in [
    ...
    def _infer_workload_type(self, query: str) -> WorkloadType:
        ...
        if any(pattern in query_upper for pattern in [
            'SIMILARITY', 'VECTOR', 'EMBEDDING', 'SEMANTIC'
        ]):
            return WorkloadType.SEMANTIC
        
        # Analytics patterns
        if any(pattern in query_upper for pattern in [
            'GROUP BY', 'HAVING', 'WINDOW', 'AGGREGATE', 'TIME_SERIES'
        ]):
    ...
    def _route_query(self, query: str, complexity: QueryComplexity, 
        ...
        query_signature = f"{complexity.value}_{workload_type.value}_{hash(query) % 1000}"
        ...
        # Analytics routing
        elif (complexity in [QueryComplexity.COMPLEX, QueryComplexity.ANALYTICAL] and 
              workload_type == WorkloadType.ANALYTICAL and self._duckdb_engine):
            target_engine = "duckdb"
        
        # High-performance read routing to DuckDB
        elif (complexity == QueryComplexity.MODERATE and 
              workload_type == WorkloadType.TRANSACTIONAL and
              'SELECT' in query.upper() and 'INSERT' not in query.upper() and
              self._duckdb_engine and self._workload_metrics.read_ratio > 0.8):
            target_engine = "duckdb"
    ...
    def _execute_on_engine(self, engine_name: str, query: str, 
                          params: Optional[Dict], context: Optional[QueryContext]) -> DatabaseResult:
    ...
    def _should_enable_duckdb(self) -> bool:
        ...
        return (
            self._workload_metrics.aggregation_frequency > 0.1 or
            self._workload_metrics.avg_result_size > 100 or
            config.debug_mode  # Always enable in debug for testing
        )
    ...
    def _should_enable_faiss(self) -> bool:
        ...
        return (
            config.enable_decision_intelligence or  # Decision intelligence needs semantic search
            config.debug_mode  # Always enable in debug for testing
        )
    
    def _record_query_performance(self, query: str, execution_time: float,
                                 engine: str, result: DatabaseResult):
    ...
    def _update_workload_metrics(self, query: str, execution_time: float, 
        ...
        has_aggregation = any(agg in query_upper for agg in [
    ...
    def _get_workload_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'aggregation_frequency': round(self._workload_metrics.aggregation_frequency, 3),
            'json_query_frequency': round(self._workload_metrics.json_query_frequency, 3),
    ...
    def _calculate_routing_efficiency(self) -> Dict[str, Any]:
        ...
        sla_compliance_rate = sla_compliant / total_queries if total_queries > 0 else 0.0
        ...
        for engine in engine_usage:
            ...
            if engine_metrics:
                avg_time = sum(m['execution_time_ms'] for m in engine_metrics) / len(engine_metrics)
                engine_performance[engine] = round(avg_time, 2)
        
        return {
            'sla_compliance_rate': round(sla_compliance_rate, 3),
            'engine_utilization': {k: round(v/total_queries, 3) for k, v in engine_usage.items()},
    ...
    def _update_routing_strategy(self, optimization_results: List[Tuple[str, bool]]):
        ...
        for engine, success in optimization_results:
            if success:
                self.logger.info(f"{engine} optimization successful, prioritizing for routing")
            else:
                self.logger.warning(f"{engine} optimization failed, deprioritizing for routing")
