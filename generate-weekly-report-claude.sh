...
pull_team_epics() {
    if [[ "$LIVE_DATA" == "true" ]]; then
        ...
        curl -s -u "user1@company.com:$JIRA_API_TOKEN" \
            -H "Accept: application/json" \
            "https://***REMOVED***/rest/api/3/search?jql=$(echo "$JIRA_QUERY" | sed 's/ /%20/g')&maxResults=100" | \
...
generate_business_value() {
    ...
    case "$summary" in
        *"FedRAMP"*|*"Security"*|*"Vulnerability"*)
            ...
            ;;
        *"***REMOVED*** Explore"*|*"GA"*|*"General Availability"*)
...
generate_team_content() {
    ...
    if [[ "$LIVE_DATA" == "true" && -f "/tmp/ui_foundation_epics.txt" ]]; then
        ...
        grep "^$project_key:" /tmp/ui_foundation_epics.txt | head -3 | while read -r line; do
            ...
            echo "- **[$epic_key](https://***REMOVED***/browse/$epic_key)**: \"$summary\" ✅"
...
if [[ "$LIVE_DATA" == "true" && -f "/tmp/ui_foundation_epics.txt" ]]; then
    ...
    echo "*Data sources: ***REMOVED*** Jira API ($epic_count completed epics analyzed across 7 UI Foundation teams)*" >> "$OUTPUT_FILE"
else
