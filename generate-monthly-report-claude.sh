...
pull_ui_foundation_initiatives() {
    if [[ "$LIVE_DATA" == "true" ]]; then
        ...
        curl -s -u "user1@company.com:$JIRA_API_TOKEN" \
            -H "Accept: application/json" \
            -G "https://***REMOVED***/rest/api/3/search" \
...
### $status_indicator [$key](https://***REMOVED***/browse/$key)
**Initiative**: $summary  
...
### 🟢 [PI-XXXXX](https://***REMOVED***/browse/PI-XXXXX)
...
*📊 **Data Integrity**: All metrics sourced from ***REMOVED*** Jira PI project. No invented numbers or estimates.*
