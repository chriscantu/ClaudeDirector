...
class DailyArchitectureAlerts:
    ...
    def generate_daily_alerts(self) -> bool:
        ...
        try:
            ...
            elif readiness_score >= self.alert_thresholds['migration_readiness_warning']:
                self._generate_warning_migration_alert(assessment)
            elif readiness_score >= self.alert_thresholds['migration_readiness_monitor']:
            ...
            print("   python memory/architecture_health_monitor.py --assess  # Full assessment")
            print("   python memory/architecture_health_monitor.py --trends  # Trend analysis")
    ...
    def _generate_critical_migration_alert(self, assessment: Dict):
        ...
        print("   ClaudeDirector's SQLite database is approaching architectural limits.")
        print()
    ...
    def _generate_warning_migration_alert(self, assessment: Dict):
        ...
        print("   Consider planning embedded database migration in the next 1-3 months.")
        print()
    ...
    def _show_criteria_status(self, criteria_analysis: Dict):
        ...
        if 'performance' in criteria_analysis:
            ...
            if 'slow_queries' in perf:
                ...
                print(f"     {status_icon} Slow Queries: {sq['current']:.1f}% (threshold: {sq['warning_threshold']:.1f}%)")
            
            if 'avg_query_time' in perf:
                qt = perf['avg_query_time']
                status_icon = self._get_status_icon(qt['status'])
                print(f"     {status_icon} Avg Query Time: {qt['current']:.3f}s (threshold: {qt['warning_threshold']:.1f}s)")
        ...
        if 'scale' in criteria_analysis:
            ...
            if 'database_size' in scale:
                ...
                print(f"     {status_icon} Database Size: {ds['current_mb']:.1f}MB (threshold: {ds['warning_threshold_gb']:.1f}GB)")
            
            if 'stakeholder_count' in scale:
                sc = scale['stakeholder_count']
                status_icon = self._get_status_icon(sc['status'])
                print(f"     {status_icon} Stakeholders: {sc['current']} (threshold: {sc['warning_threshold']})")
        ...
        if 'organizational' in criteria_analysis:
            ...
            if 'user_count' in org:
                ...
                print(f"     {status_icon} Estimated Users: {uc['current']} (threshold: {uc['warning_threshold']})")
